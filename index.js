...
if (!GROQ_API_KEY) {
  console.warn("WARNING: No GROQ_API_KEY found. Ticket AI will be disabled.");
}
...
const NAVY_COLOR    = 0x3498db;
const MINT_COLOR    = 0x00b894;
...
// ─────────────────────────────────────────────
//  Groq AI — free model list (fastest free inference)
// ─────────────────────────────────────────────
const GROQ_MODELS = [
...
- If someone is upset or frustrated, be empathetic and reassure them staff will help
- Keep responses concise and well-formatted — use bullet points when listing things
- You can share brief Lua code snippets or scripting help if it genuinely assists the customer
...
- If someone is rude, calmly redirect: "Let's keep things respectful — staff will be here to help soon 💗"
...
- "How do I pay?" → PayPal Friends & Family, CashApp, Robux group payout (you cover the 30% tax), or gift cards
- "Can I get a refund?" → All sales are final — no refunds. See our Terms of Service for details
- "How long will it take?" → Depends on the complexity of your request — staff will give you a proper estimate
- "What can you make?" → Almost anything in Roblox: game systems, UI/GUIs, admin panels, datastores, anti-cheat, weapons, vehicles, cutscenes, leaderboards, and more
- "How much does it cost?" → Pricing is based on complexity — staff will quote you after reviewing what you need
- "Can I see examples?" → Use \`s!work\` to browse our portfolio
- "Is this legit?" → Absolutely! Check reviews with \`s!review\` and portfolio with \`s!work\`
- "What are your terms?" → Read here: https://docs.google.com/document/d/13dYCdrmj9mU9jWmEYONExBDnfcwYlQfxR6UJdZRsP28/edit
- "Privacy policy?" → Read here: https://docs.google.com/document/d/16ppiOOtWtPkarJVjmxbsHBlObJFQg8IIrnMOaFhDcXk/edit
- "Partnership?" → Ask for: server name, invite link, member count, server focus, and what you can offer in return`;

const AI_CHAT_PROMPT = `You are a smart, friendly AI assistant. You can help with absolutely anything — questions, writing, ideas, coding in any language, math, creative projects, advice, explanations, or just chatting.

Be conversational, clear, and genuinely useful. Keep responses well-structured and easy to read. Use formatting (bullet points, code blocks, bold text) when it makes your answer clearer.

You are NOT limited to any specific topic. Answer freely, naturally, and accurately — like a knowledgeable friend would. Do not mention tickets, orders, or commissions unless the person specifically brings it up.`;

async function callTicketAI(channelId, userMessage, ticketType, formContext) {
  if (!GROQ_API_KEY) return null;

  if (!ticketConversations.has(channelId)) {
    ticketConversations.set(channelId, []);
  }
  ...
  for (const model of GROQ_MODELS) {
    try {
      ...
      console.error(`[AI] Fetch error with model ${model}:`, err.message);
      continue;
    }
  }
...
function getOrderStatusInfo(status) {
  return ORDER_STATUSES[status] || { label: status, color: BRAND_COLOR };
}
...
function getAntiRaidSettings(guildId) {
  if (!data.antiRaid[guildId]) {
    data.antiRaid[guildId] = {
      ...
      minAccountAge: 0, logAction: true, autoUnlock: 30_000,
...
function getAntiNukeSettings(guildId) {
  if (!data.antiNuke[guildId]) {
    data.antiNuke[guildId] = {
      ...
      logAction: true, notifyChannel: null,
    };
  }
...
async function handleAntiRaidJoin(member) {
  ...
  if (settings.whitelistedRoles?.length) {
    const hasWhitelisted = member.roles.cache.some(r => settings.whitelistedRoles.includes(r.id));
  ...
  if (settings.minAccountAge > 0) {
    ...
    const minMs = settings.minAccountAge * TIME_UNITS.d;
    if (accountAgeMs < minMs) {
      try {
        const dmMsg = `🛡️ **${member.guild.name} — Anti-Raid Protection**\n\nYour account is too new to join this server (minimum age: **${settings.minAccountAge} day(s)**). Please try again later.`;
        ...
        await sendRaidAlert(member.guild, settings, {
          ...
          fields: [
            ...
            { name: "📋 Min Age",     value: `${settings.minAccountAge} day(s)`,  inline: true },
          ],
        });
      } catch {}
      return;
  ...
  if (tracker.joins.length >= settings.threshold && !tracker.locked) {
    ...
    await sendRaidAlert(member.guild, settings, {
      ...
      description: `**${tracker.joins.length}** joins in **${settings.window / 1000}s** — triggering **${settings.action}** protocol.`,
      fields: [
        { name: "⚡ Action", value: settings.action,              inline: true },
        { name: "📊 Joins",  value: `${tracker.joins.length}`,    inline: true },
    ...
    if (settings.action === "lockdown") {
      ...
      if (settings.autoUnlock > 0) {
        setTimeout(async () => {
          if (raidTracker.has(member.guild.id)) raidTracker.get(member.guild.id).locked = false;
          antiRaidLockdowns.delete(member.guild.id);
          const unlockChannels = member.guild.channels.cache.filter(c => c.type === ChannelType.GuildText);
          for (const [, ch] of unlockChannels) await ch.permissionOverwrites.edit(member.guild.roles.everyone, { SendMessages: null }).catch(() => {});
          await sendRaidAlert(member.guild, settings, { title: "🔓 Lockdown Lifted", description: `Auto-unlocked after ${formatDuration(settings.autoUnlock)}.`, danger: false });
        }, settings.autoUnlock);
      }
    }
    if (settings.autoUnlock > 0 && settings.action !== "lockdown") {
      setTimeout(() => { if (raidTracker.has(member.guild.id)) raidTracker.get(member.guild.id).locked = false; }, settings.autoUnlock);
    }
  }
  if (tracker.locked && (settings.action === "kick" || settings.action === "ban")) {
    try {
      const dmReason = settings.action === "kick"
        ? `You were removed from **${member.guild.name}** by the anti-raid system. You may rejoin once the raid protection has lifted.`
        : `You were banned from **${member.guild.name}** by the anti-raid system due to a detected raid. Contact the server owner if you believe this is a mistake.`;
      await member.user.send({ content: `🛡️ **${member.guild.name} — Anti-Raid Protection**\n\n${dmReason}` }).catch(() => {});
      if (settings.action === "kick") await member.kick("Anti-Raid: raid detected");
...
async function nukeResponse(guild, userId, reason, settings) {
  ...
  if (settings.trustedRoles?.length) {
    const hasTrusted = member.roles.cache.some(r => settings.trustedRoles.includes(r.id));
    if (hasTrusted) return;
  }
  let actionTaken = "none";
  try {
    const dmMap = {
      ban:         `You have been **banned** from **${guild.name}** by the anti-nuke system.\n**Reason:** ${reason}`,
      kick:        `You have been **kicked** from **${guild.name}** by the anti-nuke system.\n**Reason:** ${reason}`,
      strip_roles: `Your roles have been **stripped** in **${guild.name}** by the anti-nuke system.\n**Reason:** ${reason}`,
    };
    if (dmMap[settings.action]) {
      await member.user.send({ content: `🚨 **${guild.name} — Anti-Nuke System**\n\n${dmMap[settings.action]}\n\nIf you believe this is a mistake, please contact the server owner.` }).catch(() => {});
    }
    if (settings.action === "ban")          { await member.ban({ reason: `Anti-Nuke: ${reason}` }); actionTaken = "Banned"; }
      ...
      const manageable = member.roles.cache.filter(r => r.id !== guild.id && r.manageable);
      for (const [, r] of manageable) await member.roles.remove(r).catch(() => {});
      actionTaken = "Roles Stripped";
    }
  } catch {}
  await sendNukeAlert(guild, settings, {
    title: "🚨 ANTI-NUKE TRIGGERED",
    description: `**${member.user.tag}** performed suspicious activity and was actioned: **${actionTaken}**`,
    fields: [
...
async function handleAntiRaid(message, args) {
  ...
  if (sub === "enable") {
    settings.enabled = true; saveData();
    return respond(message, { embeds: [successEmbed("🛡️ Anti-Raid Enabled")
      .addFields(
        { name: "📊 Threshold",   value: `${settings.threshold} joins`,      inline: true },
        { name: "⏱️ Window",      value: `${settings.window / 1000}s`,        inline: true },
        { name: "⚡ Action",      value: settings.action,                     inline: true },
        { name: "📅 Min Age",     value: settings.minAccountAge ? `${settings.minAccountAge}d` : "Disabled", inline: true },
        { name: "🔓 Auto-Unlock", value: settings.autoUnlock ? formatDuration(settings.autoUnlock) : "Manual", inline: true },
      ).setTimestamp()] });
  }
  if (sub === "disable") {
    settings.enabled = false; saveData();
    return respond(message, { embeds: [warnEmbed("🛡️ Anti-Raid Disabled").setTimestamp()] });
  }
  if (sub === "config") {
    ...
    if (param === "threshold") {
      ...
      if (!Number.isFinite(n) || n < 2) return respond(message, { embeds: [errorEmbed("Invalid").setDescription("Threshold must be ≥ 2.")] });
      settings.threshold = n; saveData();
      return respond(message, { embeds: [successEmbed("✅ Threshold Updated").setDescription(`Raid triggers after **${n}** joins.`)] });
    }
    if (param === "window") {
      const n = parseInt(val, 10) * 1000;
      if (!Number.isFinite(n) || n < 1000) return respond(message, { embeds: [errorEmbed("Invalid")] });
      settings.window = n; saveData();
      return respond(message, { embeds: [successEmbed("✅ Window Updated").setDescription(`Detection window: **${val}s**.`)] });
    }
    if (param === "action") {
      if (!["kick", "ban", "lockdown"].includes(val)) return respond(message, { embeds: [errorEmbed("Invalid").setDescription("Actions: `kick`, `ban`, `lockdown`")] });
      settings.action = val; saveData();
      return respond(message, { embeds: [successEmbed("✅ Action Updated").setDescription(`Action on raid: **${val}**`)] });
    }
    if (param === "minage") {
      const n = parseInt(val, 10);
      if (!Number.isFinite(n) || n < 0) return respond(message, { embeds: [errorEmbed("Invalid")] });
      settings.minAccountAge = n; saveData();
      return respond(message, { embeds: [successEmbed("✅ Min Age Updated").setDescription(n === 0 ? "Disabled." : `Accounts under **${n}d** old will be actioned.`)] });
    }
    if (param === "autounlock") {
      const n = parseInt(val, 10) * 1000;
      settings.autoUnlock = n || 0; saveData();
      return respond(message, { embeds: [successEmbed("✅ Auto-Unlock Updated").setDescription(n ? `Auto-unlock after **${val}s**.` : "Manual only.")] });
    }
    if (param === "alertchannel") {
      const chId = parseChannelId(val);
      settings.notifyChannel = chId || null; saveData();
      return respond(message, { embeds: [successEmbed("✅ Alert Channel Set").setDescription(chId ? `Alerts → <#${chId}>` : "Using mod log channel.")] });
    }
    if (param === "whitelistrole") {
      const roleId = parseRoleId(val);
      if (!roleId) return respond(message, { embeds: [errorEmbed("Invalid Role")] });
      if (!settings.whitelistedRoles) settings.whitelistedRoles = [];
      const idx = settings.whitelistedRoles.indexOf(roleId);
      if (idx === -1) { settings.whitelistedRoles.push(roleId); saveData(); return respond(message, { embeds: [successEmbed("✅ Role Whitelisted")] }); }
      else { settings.whitelistedRoles.splice(idx, 1); saveData(); return respond(message, { embeds: [warnEmbed("✅ Role Removed from Whitelist")] }); }
    }
    return respond(message, { embeds: [infoEmbed("⚙️ Anti-Raid Config Options")
      .addFields(
        { name: "`config threshold <n>`",              value: "Joins needed to trigger (default: 8)" },
        { name: "`config window <secs>`",               value: "Detection window in seconds (default: 10)" },
        { name: "`config action <kick|ban|lockdown>`",  value: "Action on raid detection" },
        { name: "`config minage <days>`",               value: "Min account age in days (0 = off)" },
        { name: "`config autounlock <secs>`",           value: "Auto-unlock lockdown delay (0 = manual)" },
        { name: "`config alertchannel <#ch>`",          value: "Channel for raid alerts" },
        { name: "`config whitelistrole <@role>`",       value: "Toggle a role from anti-raid whitelist" },
      )] });
  }
  if (sub === "status") {
    const lockdown = antiRaidLockdowns.get(message.guild.id);
    return respond(message, { embeds: [infoEmbed("🛡️ Anti-Raid Status")
      ...
      .addFields(
        ...
        { name: "📅 Min Age",       value: settings.minAccountAge ? `${settings.minAccountAge}d` : "Disabled", inline: true },
        { name: "🔓 Auto-Unlock",   value: settings.autoUnlock ? formatDuration(settings.autoUnlock) : "Manual", inline: true },
        { name: "🔔 Alert Channel", value: settings.notifyChannel ? `<#${settings.notifyChannel}>` : "Mod log", inline: true },
        { name: "🏳️ Whitelisted Roles", value: settings.whitelistedRoles?.length ? settings.whitelistedRoles.map(r => `<@&${r}>`).join(", ") : "None" },
        { name: "🔒 Lockdown",      value: lockdown ? `Active since <t:${Math.floor(lockdown.lockedAt / 1000)}:R>` : "Not active" },
      ).setTimestamp()] });
  }
  if (sub === "unlock") {
    ...
    if (raidTracker.has(message.guild.id)) raidTracker.get(message.guild.id).locked = false;
  ...
  return respond(message, { embeds: [brandEmbed("🛡️ Anti-Raid System")
    .setDescription(`**Status:** ${settings.enabled ? "🟢 Active" : "🔴 Disabled"}\n**Action:** ${settings.action} | **Threshold:** ${settings.threshold} joins / ${settings.window / 1000}s`)
    .addFields({ name: "📋 Commands", value:
      `\`${PREFIX}antiraid enable\` — Enable\n\`${PREFIX}antiraid disable\` — Disable\n\`${PREFIX}antiraid status\` — Status\n\`${PREFIX}antiraid unlock\` — Lift lockdown\n\`${PREFIX}antiraid config <param> <value>\` — Configure`
    }).setTimestamp()] });
}

// ─────────────────────────────────────────────
//  Anti-Nuke command handler
// ─────────────────────────────────────────────
async function handleAntiNuke(message, args) {
  ...
  if (sub === "enable") {
    settings.enabled = true; saveData();
    return respond(message, { embeds: [successEmbed("🛡️ Anti-Nuke Enabled")
      .addFields(
  ...
  if (sub === "disable") {
    settings.enabled = false; saveData();
    return respond(message, { embeds: [warnEmbed("🛡️ Anti-Nuke Disabled").setTimestamp()] });
  }
  if (sub === "config") {
    const param = (args[1] || "").toLowerCase();
    const val   = args[2];
    ...
    if (thresholds[param]) {
      const n = parseInt(val, 10);
      if (!Number.isFinite(n) || n < 1) return respond(message, { embeds: [errorEmbed("Invalid number.")] });
      settings[thresholds[param]] = n; saveData();
      return respond(message, { embeds: [successEmbed("✅ Threshold Updated").setDescription(`**${param}** threshold: **${n}**`)] });
    }
    if (param === "window") {
      const n = parseInt(val, 10) * 1000;
      if (!Number.isFinite(n) || n < 1000) return respond(message, { embeds: [errorEmbed("Invalid")] });
    ...
    if (param === "action") {
      if (!["ban", "kick", "strip_roles"].includes(val)) return respond(message, { embeds: [errorEmbed("Invalid").setDescription("Actions: `ban`, `kick`, `strip_roles`")] });
      settings.action = val; saveData();
      return respond(message, { embeds: [successEmbed("✅ Action Updated").setDescription(`Nuke response: **${val}**`)] });
    }
    if (param === "alertchannel") {
      const chId = parseChannelId(val);
      settings.notifyChannel = chId || null; saveData();
      return respond(message, { embeds: [successEmbed("✅ Alert Channel Set")] });
    }
    if (param === "trustrole") {
      const roleId = parseRoleId(val);
      if (!roleId) return respond(message, { embeds: [errorEmbed("Invalid Role")] });
      if (!settings.trustedRoles) settings.trustedRoles = [];
      const idx = settings.trustedRoles.indexOf(roleId);
      if (idx === -1) { settings.trustedRoles.push(roleId); saveData(); return respond(message, { embeds: [successEmbed("✅ Role Trusted")] }); }
      else { settings.trustedRoles.splice(idx, 1); saveData(); return respond(message, { embeds: [warnEmbed("✅ Role Removed from Trust")] }); }
    }
    if (param === "trustuser") {
      const userId = parseUserId(val);
      if (!userId) return respond(message, { embeds: [errorEmbed("Invalid User")] });
      if (!settings.trustedUsers) settings.trustedUsers = [];
      const idx = settings.trustedUsers.indexOf(userId);
      if (idx === -1) { settings.trustedUsers.push(userId); saveData(); return respond(message, { embeds: [successEmbed("✅ User Trusted")] }); }
      else { settings.trustedUsers.splice(idx, 1); saveData(); return respond(message, { embeds: [warnEmbed("✅ User Removed from Trust")] }); }
    }
    return respond(message, { embeds: [infoEmbed("⚙️ Anti-Nuke Config Options")
      .addFields(
        { name: "`config channels <n>`",               value: "Channel delete threshold" },
        { name: "`config bans <n>`",                   value: "Ban threshold" },
        { name: "`config roles <n>`",                  value: "Role delete threshold" },
        { name: "`config webhooks <n>`",               value: "Webhook delete threshold" },
        { name: "`config kicks <n>`",                  value: "Kick threshold" },
        { name: "`config window <secs>`",              value: "Detection window in seconds" },
        { name: "`config action <ban|kick|strip_roles>`", value: "Action on trigger" },
        { name: "`config alertchannel <#ch>`",         value: "Channel for nuke alerts" },
        { name: "`config trustrole <@role>`",          value: "Toggle a role as trusted" },
        { name: "`config trustuser <@user>`",          value: "Toggle a user as trusted" },
      )] });
  }
  if (sub === "status") {
  ...
  return respond(message, { embeds: [brandEmbed("🛡️ Anti-Nuke System")
    .setDescription(`**Status:** ${settings.enabled ? "🟢 Active" : "🔴 Disabled"}`)
    .addFields({ name: "📋 Commands", value:
      `\`${PREFIX}antinuke enable\` — Enable\n\`${PREFIX}antinuke disable\` — Disable\n\`${PREFIX}antinuke status\` — Status\n\`${PREFIX}antinuke config <param> <value>\` — Configure`
    }).setTimestamp()] });
}
...
async function handleTicketRole(message, args) {
  ...
  if (sub === "list") {
    return respond(message, { embeds: [infoEmbed("🎭 Ticket Staff Roles")
      .setDescription(settings.ticketStaffRoles.length ? settings.ticketStaffRoles.map(r => `• <@&${r}>`).join("\n") : "No staff roles configured.")] });
  }
  return respond(message, { embeds: [infoEmbed("🎭 Ticket Role Management")
    .addFields(
      { name: `\`${PREFIX}ticketrole add <@role>\``,    value: "Give a role access to all tickets" },
...
async function handleTicketConfig(message, args) {
  ...
  if (sub === "ai") {
    ...
    return respond(message, { embeds: [infoEmbed("🤖 Ticket AI").setDescription(`Currently: **${settings.ticketAI !== false ? "Enabled" : "Disabled"}**\nUse \`${PREFIX}ticketconfig ai on/off\``)] });
  }
  if (sub === "greeting") {
  ...
  if (sub === "category") {
    ...
    if (!catId) { delete settings.ticketCategoryId; saveData(); return respond(message, { embeds: [successEmbed("✅ Category Cleared")] }); }
    settings.ticketCategoryId = catId; saveData();
    return respond(message, { embeds: [successEmbed("✅ Ticket Category Set").setDescription(`Tickets created under <#${catId}>.`)] });
  }
  if (sub === "maxopen") {
    const n = parseInt(args[1], 10);
    if (!Number.isFinite(n) || n < 1) return respond(message, { embeds: [errorEmbed("Invalid")] });
    settings.maxOpenTickets = n; saveData();
    return respond(message, { embeds: [successEmbed("✅ Max Open Tickets Set").setDescription(`Max **${n}** open ticket(s) per user.`)] });
  }
  return respond(message, { embeds: [brandEmbed("⚙️ Ticket Configuration")
    .addFields(
      ...
      { name: `\`${PREFIX}ticketconfig category <#ch>\``,  value: `Ticket category — **${settings.ticketCategoryId || "None"}**` },
      { name: `\`${PREFIX}ticketconfig maxopen <n>\``,     value: `Max open tickets/user — **${settings.maxOpenTickets || 1}**` },
      { name: `\`${PREFIX}ticketrole add/remove/list\``,   value: "Manage staff roles for ticket access" },
    ).setTimestamp()] });
}
...
async function updateOrderChannel() {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`**Last updated:** <t:${Math.floor(Date.now() / 1000)}:R>\n**Active Orders:** ${activeOrders.length} | **Completed Today:** ${completedToday.length}\n\n> Open a ticket to place a commission! 💗`)
      .setFooter({ text: `${BOT_NAME} • Updates every 3 hours` })
      .setTimestamp();
    if (activeOrders.length === 0) {
      embed.addFields({ name: "✅ Queue Clear", value: "No active orders right now!" });
    } else {
      for (const order of activeOrders.slice(0, 10)) {
        const statusInfo = getOrderStatusInfo(order.status);
        embed.addFields({
          name: `${statusInfo.label} — Order #${order.id}`,
          value: `**Customer:** <@${order.userId}>\n**Details:** ${order.details.slice(0, 100)}${order.details.length > 100 ? "..." : ""}\n**Updated:** <t:${Math.floor(new Date(order.updatedAt).getTime() / 1000)}:R>`,
        });
      }
      if (activeOrders.length > 10) embed.addFields({ name: `...and ${activeOrders.length - 10} more`, value: "Use `s!orderinfo <id>` to check any order." });
    }
    const settings = getGuildSettings(HOME_GUILD_ID);
    if (settings.orderBoardMessageId) { try { await ch.messages.delete(settings.orderBoardMessageId); } catch {} }
    const sent = await ch.send({ embeds: [embed] });
    settings.orderBoardMessageId = sent.id;
    saveData();
...
async function repostTicketPanels() {
  for (const [guildId, settings] of Object.entries(data.settings)) {
    ...
    try {
      ...
      saveData();
    } catch (err) { console.error(`Ticket panel repost failed for guild ${guildId}:`, err); }
  }
}
...
function buildTicketPanelEmbed() {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      "**Need help? Want to commission something?**\nChoose the option that best fits your needs below!\n\n" +
      "╔══════════════════════════════╗\n" +
      "║  📦 **Order** — Commission a script/system\n" +
      "║  🤝 **Partnership** — Partner with our server\n" +
      "║  ❓ **Inquiry** — Questions, support, help\n" +
      "╚══════════════════════════════╝\n\n" +
      "*A staff member will assist you shortly. Average response time: 24 hours.*"
    )
    .setFooter({ text: `${BOT_NAME} • Ticket System v4 • AI-powered support` })
    .setTimestamp();
...
async function sendSetupMessage(guild) {
  ...
  const embed = new EmbedBuilder()
    .setColor(BRAND_COLOR)
    .setTitle(`🧸 ${BOT_NAME} v${BOT_VERSION} — Welcome & Setup Guide`)
    .setDescription(`Thanks for adding **${BOT_NAME}** to **${guild.name}**!\n\nAll commands use the **\`${PREFIX}\`** prefix.`)
    .addFields(
      { name: "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", value: "**⚙️ Required Setup**" },
      { name: "Step 1 — Mod Log Channel",     value: `\`${PREFIX}setlog #channel\`` },
      { name: "Step 2 — Reviews Channel",     value: `\`${PREFIX}setreviews #channel\`` },
      { name: "Step 3 — Transcripts Channel", value: `\`${PREFIX}settranscripts #channel\`` },
      { name: "Step 4 — Ticket Panel",        value: `\`${PREFIX}ticketpanel\`` },
      { name: "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", value: "**🎫 Ticket Configuration**" },
      { name: "Staff Roles",  value: `\`${PREFIX}ticketrole add @role\`` },
      { name: "AI Assistant", value: `\`${PREFIX}ticketconfig ai on\`` },
      { name: "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", value: "**🛡️ Security**" },
      { name: "Anti-Raid", value: `\`${PREFIX}antiraid enable\`` },
      { name: "Anti-Nuke", value: `\`${PREFIX}antinuke enable\`` },
    )
...
async function handleXpGrant(message) {
  ...
  if (lv.xp >= needed) {
    ...
    try {
      ...
      await ch.send({ content: `<@${userId}>`, embeds: [new EmbedBuilder()
...
async function handleWork(message) {
  const wait = checkCooldown("work", message.author.id);
  if (wait > 0) return respond(message, { embeds: [warnEmbed("⏰ Cooldown").setDescription(`Work again in **${formatDuration(wait * 1000)}**.`)] });
  consumeCooldown("work", message.author.id);
  const eco = getEconomy(message.author.id);
...
async function handleBuy(message, args) {
  ...
  saveData();
  await respond(message, { embeds: [successEmbed("✅ Purchase Successful!")
    .addFields({ name: "🛍️ Item", value: item.name }, { name: "📋 How to redeem", value: item.desc }).setTimestamp()] });
}
...
async function handleInviteLeaderboard(message) {
  ...
  const entries = Object.entries(guildInvites)
    ...
    .sort((a, b) => (b.invited - b.left) - (a.invited - a.left)).slice(0, 10);
...
async function handlePartner(message, args) {
  ...
  if (subcommand === "help" || !subcommand) {
    return respond(message, { embeds: [infoEmbed("🤝 Partnership System")
      .addFields(
  ...
  if (subcommand === "list") {
    ...
    return respond(message, { embeds: [infoEmbed(`🤝 Partnerships — ${ps.length}`).setDescription(ps.map(p => `**#${p.id}** [${p.name || "Unnamed"}](${p.invite}) — by ${p.addedBy}`).join("\n"))] });
  }
  ...
  if (subcommand === "basic") {
    if (parts.length < 3) return respond(message, { embeds: [warnEmbed("Not Enough Info")] });
    embed = new EmbedBuilder().setColor(INFO_COLOR).setTitle("🤝 New Partnership!")
      ...
      .addFields({ name: "🏠 Server", value: `**${parts[1]}**`, inline: true }, { name: "🔗 Join Now", value: `[Click here!](${invite})`, inline: true }, { name: "📋 About", value: parts[2] || "No description." })
    ...
    embed = new EmbedBuilder().setColor(BRAND_COLOR).setTitle("🌟 Featured Partner")
      ...
      .addFields({ name: "🏠 Server", value: `**${parts[1] || "—"}**`, inline: true }, { name: "🔗 Join", value: `[${parts[1] || "Join"}](${invite})`, inline: true }, { name: "📖 About", value: parts[2] || "—" });
    ...
    if (parts.length < 5) return respond(message, { embeds: [warnEmbed("Not Enough Info")] });
    embed = new EmbedBuilder().setColor(GOLD_COLOR).setTitle("🤝 Official Partnership Announcement")
      ...
      .addFields({ name: "🏠 Server", value: `**${parts[1]}**`, inline: true }, { name: "🔗 Join", value: `[Click here](${invite})`, inline: true }, { name: "📋 About Them", value: parts[2] || "—" }, { name: "🎁 What They Offer", value: parts[3] || "—", inline: true }, { name: "💜 What We Offer", value: parts[4] || "—", inline: true })
      .setFooter({ text: `Partnership • ${footerTag}` }).setTimestamp();
  } else if (subcommand === "promo") {
    if (parts.length < 3) return respond(message, { embeds: [warnEmbed("Not Enough Info")] });
    embed = new EmbedBuilder().setColor(SUCCESS_COLOR).setTitle(`📣 Shoutout — ${parts[1]}`).setDescription(parts[2])
      .addFields({ name: "🔗 Join Server", value: invite }).setFooter({ text: `Promo by ${footerTag}` }).setTimestamp();
  } else {
    return respond(message, { embeds: [warnEmbed("Unknown Format")] });
  }
  data.partnerships.push({ id: data.partnerships.length ? Math.max(...data.partnerships.map(p => p.id)) + 1 : 1, guildId: message.guild.id, invite, name: parts[1] || null, format: subcommand, addedBy: message.author.tag, addedAt: new Date().toISOString() });
  saveData();
  try { await message.delete(); } catch {}
  await message.channel.send({ content: "@here — New partnership!", embeds: [embed] });
}
...
async function handleAnnounce(message, args) {
  ...
  if (!text) return respond(message, { embeds: [warnEmbed("Usage").setDescription(`\`${PREFIX}announce [#channel] <message>\``)] });
  const embed = new EmbedBuilder().setColor(BRAND_COLOR)
    ...
    .setDescription(`╔══════════════════════════════════╗\n\u200b\n${text}\n\u200b\n╚══════════════════════════════════╝`)
    .setFooter({ text: `Posted by ${message.author.tag}`, iconURL: message.author.displayAvatarURL() }).setTimestamp();
  try { await message.delete(); } catch {}
...
async function handleOrderInfo(message, args) {
  ...
  const statusInfo = getOrderStatusInfo(order.status);
  const embed = new EmbedBuilder().setColor(statusInfo.color).setTitle(`📦 Order #${order.id}`)
    .addFields(
...
async function handleUpdateOrder(message, args) {
  ...
  if (!orderId || !newStatus) {
    return respond(message, { embeds: [warnEmbed("Usage").setDescription(`\`${PREFIX}updateorder <id> <status> [note]\`\n\n**Statuses:**\n${Object.entries(ORDER_STATUSES).map(([k, v]) => `\`${k}\` — ${v.label}`).join("\n")}`)] });
  }
  const order = findOrder(orderId);
  if (!order) return respond(message, { embeds: [errorEmbed("Not Found")] });
  if (!ORDER_STATUSES[newStatus]) return respond(message, { embeds: [errorEmbed("Invalid Status")] });
  ...
  const embed = new EmbedBuilder().setColor(statusInfo.color).setTitle(`✅ Order #${order.id} Updated`)
    .addFields({ name: "📊 Old", value: getOrderStatusInfo(oldStatus).label, inline: true }, { name: "📊 New", value: statusInfo.label, inline: true }, { name: "👤 Customer", value: `<@${order.userId}>`, inline: true });
  if (note) embed.addFields({ name: "📝 Note", value: note });
  embed.setTimestamp();
  await respond(message, { embeds: [embed] });
  try {
    const user = await client.users.fetch(order.userId);
    await user.send({ embeds: [new EmbedBuilder().setColor(statusInfo.color).setTitle(`📦 Your Order #${order.id} Was Updated`)
      .setDescription(`Status changed to **${statusInfo.label}**.${note ? `\n\n**Staff Note:** ${note}` : ""}`)
...
async function handleComplete(message, args) {
  ...
  saveData(); updateOrderChannel().catch(() => {});
  await respond(message, { embeds: [successEmbed(`✅ Order #${order.id} Completed`).addFields({ name: "👤 Customer", value: `<@${order.userId}>` }).setTimestamp()] });
}
...
async function handleRPS(message, args) {
  ...
  await respond(message, { embeds: [brandEmbed("🎮 Rock Paper Scissors")
    .addFields({ name: "Your Choice", value: `${emojis[userChoice]} ${userChoice}`, inline: true }, { name: "My Choice", value: `${emojis[botChoice]} ${botChoice}`, inline: true }, { name: "Result", value: outcome })] });
}
...
async function handleGiveaway(message, args) {
  ...
  if (!ms)    return respond(message, { embeds: [errorEmbed("Invalid Duration")] });
  ...
  const embed = new EmbedBuilder().setColor(GOLD_COLOR).setTitle("🎉 GIVEAWAY!")
    ...
    .addFields({ name: "⏰ Ends", value: `<t:${Math.floor(endAt / 1000)}:R>`, inline: true }, { name: "🎁 Prize", value: prize, inline: true }, { name: "🏠 Host", value: `<@${message.author.id}>`, inline: true })
    .setFooter({ text: "React with 🎉 to enter!" }).setTimestamp();
  const sent = await message.channel.send({ content: "@here", embeds: [embed], allowedMentions: { users: [] } });
  ...
  for (const [msgId, giveaway] of Object.entries(data.giveaways)) {
    ...
    try {
      ...
      if (!entrants.length) {
        ...
        await channel.send({ content: `🎉 Congratulations <@${winnerId}>!`, embeds: [successEmbed("🎉 Giveaway Winner!").setDescription(`<@${winnerId}> won **${giveaway.prize}**!\nContact staff to claim your prize.`).addFields({ name: "📊 Entries", value: `${entrants.length}` }).setTimestamp()], allowedMentions: { users: [winnerId] } });
      }
    } catch (err) { console.error("Giveaway end failed:", err); }
  }
}
...
async function handleNick(message, args) {
  ...
  let member;
  try { member = await message.guild.members.fetch(userId); } catch { return respond(message, { embeds: [errorEmbed("Not Found")] }); }
...
// ─────────────────────────────────────────────
//  s!ai — general-purpose AI chat command
// ─────────────────────────────────────────────
...
async function handleAI(message, args) {
  if (!GROQ_API_KEY) {
    return message.channel.send({ content: "❌ No `GROQ_API_KEY` is configured. Add it to `.env` and restart the bot.", allowedMentions: { users: [] } });
  }

  const prompt = args.join(" ").trim();
  if (!prompt) {
    return message.channel.send({
      content: `-# 🤖 **Snuggles AI**\n\n**Usage:** \`${PREFIX}ai <your message>\`\n\nYou can ask me anything — coding help, writing, math, creative ideas, general questions, or chat.\n\n**Examples:**\n• \`${PREFIX}ai explain how async/await works in JavaScript\`\n• \`${PREFIX}ai write a short welcome message for a Discord server\`\n• \`${PREFIX}ai generate an image of a cozy cabin in the snow\`\n• \`${PREFIX}ai what's the difference between TCP and UDP?\``,
  ...
  if (IMAGE_REQUEST_RE.test(prompt)) {
    try {
      ...
      await message.channel.send({
        content: `-# 🎨 **Snuggles AI**  •  image generation  •  asked by ${message.author.username}\n\n**Prompt:** ${prompt}\n${imageUrl}`,
        allowedMentions: { users: [] },
      });
    } catch (err) {
      console.error("[s!ai image]", err.message);
      await message.channel.send({ content: "❌ Image generation failed. Try again in a moment.", allowedMentions: { users: [] } });
    }
    return;
  }

  try {
    const sessionId = `staff_ai_${message.author.id}`;
      ...
      try {
        const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            max_tokens: 1200,
            temperature: 0.7,
            messages: [{ role: "system", content: AI_CHAT_PROMPT }, ...history],
          }),
        });

        if (!res.ok) {
          if (res.status === 429) { console.warn(`[s!ai] ${model} rate limited`); continue; }
          console.error(`[s!ai] ${model} error ${res.status}`);
          continue;
        }
        ...
        if (aiText) {
          history.push({ role: "assistant", content: aiText });
          console.log(`[s!ai] ${message.author.tag} ← ${model}`);
          break;
        }
      } catch (err) {
        console.error(`[s!ai] ${model}:`, err.message);
      }
    }

    if (!aiText) {
      return message.channel.send({ content: "❌ All AI models are currently busy. Try again in a moment.", allowedMentions: { users: [] } });
    }
    ...
    console.error("[s!ai]", err);
    await message.channel.send({ content: `❌ Something went wrong: \`${err.message}\``, allowedMentions: { users: [] } });
  }
}

// ─────────────────────────────────────────────
//  s!aioff / s!aion — toggle ticket AI per channel
// ─────────────────────────────────────────────
async function handleAIToggle(message, args, commandName) {
  ...
  if (!inTicket) {
    return respond(message, { embeds: [warnEmbed("Ticket Channels Only").setDescription("This command only works inside ticket channels.")] });
  }

  if (commandName === "aioff") {
    ticketAIDisabled.add(message.channel.id);
    return message.channel.send({ content: "🔇 Ticket AI has been **turned off** for this ticket. The AI will no longer reply here.", allowedMentions: { users: [] } });
  } else {
    ticketAIDisabled.delete(message.channel.id);
    return message.channel.send({ content: "🔊 Ticket AI has been **turned on** for this ticket.", allowedMentions: { users: [] } });
  }
}
...
async function handleHelp(message, args) {
  const sub = (args[0] || "").toLowerCase();
  const categories = {
    general:     { emoji: "📌", name: "General",     commands: [{ name: "help [category]", desc: "Show this menu." }, { name: "info", desc: "Bot info." }, { name: "ping", desc: "Latency." }, { name: "status", desc: "Systems status." }, { name: "rules", desc: "Server rules." }, { name: "uptime", desc: "Bot uptime." }, { name: "ai <message>", desc: "Chat with AI or generate images." }, { name: "aioff", desc: "Turn off AI in this ticket (staff)." }, { name: "aion", desc: "Turn AI back on in this ticket (staff)." }] },
    commissions: { emoji: "💼", name: "Commissions", commands: [{ name: "services", desc: "Everything we offer." }, { name: "prices", desc: "Pricing & payment." }, { name: "pay", desc: "Payment details." }, { name: "orderinfo <id>", desc: "Check an order." }, { name: "discount", desc: "Loyalty discount." }, { name: "ticket", desc: "How to open a ticket." }] },
    portfolio:   { emoji: "🎨", name: "Portfolio",   commands: [{ name: "work [page]", desc: "Browse previous work." }, { name: "addwork <url> [title]", desc: "Add entry (staff)." }, { name: "removework <id>", desc: "Remove entry (staff)." }] },
    scripting:   { emoji: "🔧", name: "Scripting",   commands: [{ name: "script <type>", desc: "Example scripts." }, { name: "snippet", desc: "Random Lua snippet." }, { name: "docs", desc: "Documentation." }, { name: "debug", desc: "Debug template." }, { name: "tip", desc: "Scripting tips." }] },
    leveling:    { emoji: "📊", name: "Leveling & Economy", commands: [{ name: "level [user]", desc: "Level & XP." }, { name: "rank [user]", desc: "Detailed stats." }, { name: "leaderboard", desc: "Top 10 users." }, { name: "balance [user]", desc: "Coin balance." }, { name: "work", desc: "Earn coins." }, { name: "daily", desc: "Daily reward." }, { name: "shop", desc: "Coin shop." }, { name: "buy <item_id>", desc: "Buy an item." }] },
    fun:         { emoji: "🎉", name: "Fun & Games", commands: [{ name: "quote", desc: "Motivational quote." }, { name: "meme", desc: "Random meme." }, { name: "8ball <q>", desc: "Magic 8-ball." }, { name: "rate <thing>", desc: "Rate 0–10." }, { name: "coinflip", desc: "Flip a coin." }, { name: "roll [max]", desc: "Roll a dice." }, { name: "rps <choice>", desc: "Rock paper scissors." }, { name: "trivia", desc: "Scripting trivia." }, { name: "remindme <t> | <msg>", desc: "Set a reminder." }, { name: "color <hex>", desc: "Preview a color." }, { name: "calc <expr>", desc: "Calculator." }] },
    info:        { emoji: "ℹ️", name: "Info",        commands: [{ name: "userinfo [user]", desc: "User info." }, { name: "serverinfo", desc: "Server info." }, { name: "avatar [user]", desc: "Avatar." }, { name: "banner [user]", desc: "Banner." }, { name: "servericon", desc: "Server icon." }, { name: "stats", desc: "Bot stats." }, { name: "invites [user]", desc: "Invite count." }, { name: "inviteleaderboard", desc: "Top inviters." }] },
    reviews:     { emoji: "⭐", name: "Reviews",     commands: [{ name: "review <1-5> <type> | <msg>", desc: "Submit a review." }, { name: "vouch <text>", desc: "Quick vouch." }] },
    moderation:  { emoji: "🔨", name: "Moderation",  commands: [{ name: "ban @user <reason>", desc: "Ban." }, { name: "kick @user [reason]", desc: "Kick." }, { name: "mute @user <time>", desc: "Timeout." }, { name: "warn @user <reason>", desc: "Warn." }, { name: "warns @user", desc: "View warnings." }, { name: "unwarn <id>", desc: "Remove warning." }, { name: "purge <1-100>", desc: "Bulk delete." }, { name: "lock", desc: "Lock channel." }, { name: "unlock", desc: "Unlock channel." }, { name: "slowmode <secs>", desc: "Set slowmode." }, { name: "nick @user [name]", desc: "Set nickname." }] },
    admin:       { emoji: "⚙️", name: "Admin",       commands: [{ name: "announce [#ch] <msg>", desc: "Announcement." }, { name: "partner <format> ...", desc: "Partnership." }, { name: "poll <q> | <a1> | <a2>", desc: "Reaction poll." }, { name: "giveaway <time> | <prize>", desc: "Start giveaway." }, { name: "embed <title> | <body>", desc: "Custom embed." }, { name: "addorder @user <details>", desc: "Add commission." }, { name: "updateorder <id> <status>", desc: "Update order." }, { name: "complete <id>", desc: "Mark complete." }, { name: "blacklist @user", desc: "Toggle blacklist." }, { name: "setlog [#ch]", desc: "Set mod log." }, { name: "setreviews [#ch]", desc: "Set reviews channel." }, { name: "settranscripts [#ch]", desc: "Set transcripts." }, { name: "ticketpanel", desc: "Post ticket panel." }, { name: "ticketrole add/remove/list", desc: "Manage staff roles." }, { name: "ticketconfig [option]", desc: "Configure tickets." }, { name: "close <reason>", desc: "Close ticket." }, { name: "addnote <text>", desc: "Staff note." }, { name: "givecoins @user <amt>", desc: "Give coins." }, { name: "say <message>", desc: "Send as bot." }, { name: "antiraid ...", desc: "Anti-raid system." }, { name: "antinuke ...", desc: "Anti-nuke system." }] },
  };
  if (sub && categories[sub]) {
    const cat = categories[sub];
    return respond(message, { embeds: [new EmbedBuilder().setColor(BRAND_COLOR).setTitle(`${cat.emoji} ${cat.name} Commands`)
      .setDescription(cat.commands.map(c => `\`${PREFIX}${c.name}\`\n↳ ${c.desc}`).join("\n\n"))
      .setFooter({ text: `${BOT_NAME} v${BOT_VERSION} • s!help for all categories` }).setTimestamp()] });
  }
  await respond(message, { embeds: [new EmbedBuilder().setColor(BRAND_COLOR).setTitle(`🧸 ${BOT_NAME} — Command Help`)
    .setDescription(`**Prefix:** \`${PREFIX}\` | **Version:** v${BOT_VERSION}\n\nUse \`${PREFIX}help <category>\` for detailed commands.\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`)
    .addFields(Object.entries(categories).map(([key, cat]) => ({ name: `${cat.emoji} ${cat.name}`, value: `\`${PREFIX}help ${key}\` — ${cat.commands.length} commands`, inline: true })))
    .addFields({ name: "\u200b", value: "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" }, { name: "🎫 Open a Ticket", value: "Click the buttons in the ticket panel channel!", inline: true }, { name: "💬 Support", value: "Open a General Inquiry ticket", inline: true })
...
async function handlePrices(message) {
  await respond(message, { embeds: [new EmbedBuilder().setTitle(PAYMENT_INFO.title).setDescription(PAYMENT_INFO.description).setColor(SUCCESS_COLOR)
    ...
    .addFields({ name: "⚠️ Refund Policy", value: PAYMENT_INFO.note }).setTimestamp()] });
}
...
async function handleAvatar(message, args) {
  const userId = parseUserId(args[0]) || message.author.id;
  let user;
  try { user = await client.users.fetch(userId); } catch { return respond(message, { embeds: [errorEmbed("Not Found")] }); }
...
async function handleStats(message) {
  ...
  await respond(message, { embeds: [brandEmbed("📈 Stats").addFields(
    { name: "📦 Active Orders", value: `${active}`,                        inline: true },
    { name: "✅ Completed",     value: `${completed}`,                    inline: true },
    { name: "🎟️ Tickets",      value: `${stats.ticketsOpened || 0} opened`, inline: true },
...
async function openTicketForUser(channel, member, ticketType, formAnswers) {
  ...
  if (userTickets.size >= maxOpen) return { ok: false, error: `You already have **${userTickets.size}** open ticket(s). Please use your existing ticket or wait for it to be closed. (Max: ${maxOpen})` };
  ...
  catch (err) { return { ok: false, error: "Couldn't create ticket. Check bot permissions (**Manage Channels**)." }; }
  ...
  if (settings.ticketAI !== false && GROQ_API_KEY) {
    setImmediate(async () => {
      try {
        const ticketLabel = ticketType === "partnership" ? "Partnership Manager" : ticketType === "order" ? "Order Support" : "AI Support";
        ...
        if (aiResponse) {
          await created.send({ content: `-# 🤖 **Snuggles AI**  •  ${ticketLabel}\n\n${aiResponse}`, allowedMentions: { users: [] } });
        }
      } catch (err) {
        console.error("[AI] Ticket greeting failed:", err.message);
      }
    });
  }
  ...
  if (ticketType === "partnership") {
    setImmediate(async () => {
      try {
        ...
        // Auto-decline if under 45 members
        if (memberCount > 0 && memberCount < MIN_MEMBERS) {
          const declineEmbed = new EmbedBuilder()
            .setColor(ERROR_COLOR)
            .setTitle("❌ Partnership Declined")
            .setDescription(
              `Thank you for your interest in partnering with **${BOT_NAME}**! 💗\n\n` +
              `Unfortunately we require a minimum of **${MIN_MEMBERS} members** to consider a partnership.\n\n` +
              `Your server currently has **${memberCount} member(s)**. Feel free to apply again once you've grown — we'd love to work with you in the future! 🌸`
            )
        ...
        if (memberCount >= 120) {
          ...
          tierLabel   = "🔥 Large Server (120+)";
        } else if (memberCount >= 70) {
          pingContent = "@here";
          tierLabel   = "✨ Mid-Size Server (70–119)";
        } else {
          pingContent = "";
    ...
    if (isOwner && message.content.trim().length >= 20) {
      ...
      setImmediate(async () => {
        try {
          ...
          const inviteDisplay = formAnswers.invite
            ? `[Click to Join](${formAnswers.invite})`
            : "—";

          const adEmbed = new EmbedBuilder()
            .setColor(0x9b59b6)
            .setAuthor({ name: "✨ New Partner Server", iconURL: "https://cdn.discordapp.com/emojis/1234567890.png" })
...
// ─────────────────────────────────────────────
//  Forum Support Channel — threadCreate
// ─────────────────────────────────────────────
client.on("threadCreate", async (thread) => {
  try {
    if (thread.parentId !== FORUM_SUPPORT_CHANNEL_ID) return;
    // Ensure the bot can send messages in this thread
    await thread.join().catch(() => {});

    const rulesEmbed = new EmbedBuilder()
      .setColor(BRAND_COLOR)
      .setTitle("📜 Support Forum — Welcome!")
      .setDescription(
        "Welcome to **Snuggles Scripting Support**! 💗\n\n" +
        "**Please keep these in mind:**\n" +
        "**1.** Be respectful and patient — staff will respond as soon as possible\n" +
        "**2.** Include as much detail as possible (screenshots, error messages, code snippets)\n" +
        "**3.** Stay on topic — one issue per thread\n" +
        "**4.** Mark your thread solved once your issue is resolved\n" +
        "**5.** No advertising, spam, or off-topic content\n\n" +
        "📜 [Terms of Service](https://docs.google.com/document/d/13dYCdrmj9mU9jWmEYONExBDnfcwYlQfxR6UJdZRsP28/edit) • " +
        "🔒 [Privacy Policy](https://docs.google.com/document/d/16ppiOOtWtPkarJVjmxbsHBlObJFQg8IIrnMOaFhDcXk/edit)"
      )
      .setFooter({ text: `${BOT_NAME} • Support Forum` })
      .setTimestamp();
    ...
    console.error("[Forum] threadCreate error:", err.message);
  }
});
...
client.on("guildMemberAdd", async (member) => {
  try {
    ...
    await logMod(guild, successEmbed("📥 Member Joined").addFields({ name: "User", value: `<@${member.id}> (${member.user.tag})` }, { name: "Account Age", value: `<t:${Math.floor(member.user.createdTimestamp / 1000)}:R>` }).setThumbnail(member.user.displayAvatarURL()).setTimestamp());
...
client.on("guildMemberRemove", async (member) => {
  try {
    if (data.invites[member.guild.id]) {
      for (const [, inv] of Object.entries(data.invites[member.guild.id])) { inv.left++; break; }
      saveData();
    }
    await logMod(member.guild, errorEmbed("📤 Member Left").addFields({ name: "User", value: `<@${member.id}> (${member.user.tag})` }).setThumbnail(member.user.displayAvatarURL()).setTimestamp());
  } catch {}
});
...
client.on("messageDelete", async (message) => {
  try {
    ...
    await logMod(message.guild, errorEmbed("🗑️ Message Deleted").addFields({ name: "Author", value: `<@${message.author.id}> (${message.author.tag})`, inline: true }, { name: "Channel", value: `<#${message.channel.id}>`, inline: true }, { name: "Content", value: message.content.slice(0, 1024) }).setTimestamp());
  } catch {}
});
...
client.on("messageUpdate", async (oldMessage, newMessage) => {
  try {
    ...
    await logMod(newMessage.guild, warnEmbed("✏️ Message Edited").addFields({ name: "Author", value: `<@${newMessage.author.id}> (${newMessage.author.tag})`, inline: true }, { name: "Channel", value: `<#${newMessage.channel.id}>`, inline: true }, { name: "Before", value: (oldMessage.content || "—").slice(0, 1024) }, { name: "After", value: (newMessage.content || "—").slice(0, 1024) }, { name: "Jump", value: `[Go to message](${newMessage.url})`, inline: true }).setTimestamp());
  } catch {}
});

client.on("roleCreate",  async (role)  => { try { await logMod(role.guild, new EmbedBuilder().setColor(SUCCESS_COLOR).setTitle("🎭 Role Created").addFields({ name: "Name", value: role.name, inline: true }, { name: "ID", value: role.id, inline: true }, { name: "Color", value: role.hexColor, inline: true }).setTimestamp()); } catch {} });
client.on("roleDelete",  async (role)  => {
  try {
    await logMod(role.guild, new EmbedBuilder().setColor(ERROR_COLOR).setTitle("🎭 Role Deleted").addFields({ name: "Name", value: role.name, inline: true }, { name: "ID", value: role.id, inline: true }).setTimestamp());
...
client.on("roleUpdate",  async (oldRole, newRole) => {
  try {
    ...
    await logMod(newRole.guild, new EmbedBuilder().setColor(WARN_COLOR).setTitle("🎭 Role Updated").addFields({ name: "Role", value: `<@&${newRole.id}>` }, { name: "Changes", value: changes.join("\n") }).setTimestamp());
  } catch {}
});
client.on("channelCreate", async (channel) => { if (!channel.guild) return; try { await logMod(channel.guild, new EmbedBuilder().setColor(SUCCESS_COLOR).setTitle("💬 Channel Created").addFields({ name: "Name", value: channel.name, inline: true }, { name: "ID", value: channel.id, inline: true }).setTimestamp()); } catch {} });
client.on("channelDelete", async (channel) => {
  if (!channel.guild) return;
  try {
    await logMod(channel.guild, new EmbedBuilder().setColor(ERROR_COLOR).setTitle("💬 Channel Deleted").addFields({ name: "Name", value: channel.name, inline: true }, { name: "ID", value: channel.id, inline: true }).setTimestamp());
...
client.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  try {
    ...
    if (!changes.length) return;
    await logMod(newChannel.guild, new EmbedBuilder().setColor(WARN_COLOR).setTitle("💬 Channel Updated").addFields({ name: "Channel", value: `<#${newChannel.id}>` }, { name: "Changes", value: changes.join("\n") }).setTimestamp());
  } catch {}
});
client.on("guildMemberUpdate", async (oldMember, newMember) => {
  try {
    ...
    if (!changes.length) return;
    await logMod(newMember.guild, new EmbedBuilder().setColor(INFO_COLOR).setTitle("👤 Member Updated").addFields({ name: "User", value: `<@${newMember.id}> (${newMember.user.tag})` }, { name: "Changes", value: changes.join("\n") }).setTimestamp());
  } catch {}
});
client.on("guildBanAdd", async (ban) => {
  try {
    await logMod(ban.guild, new EmbedBuilder().setColor(ERROR_COLOR).setTitle("🔨 Member Banned").addFields({ name: "User", value: `${ban.user.tag} (${ban.user.id})` }, { name: "Reason", value: ban.reason || "No reason" }).setTimestamp());
...
client.on("guildBanRemove", async (ban) => { try { await logMod(ban.guild, new EmbedBuilder().setColor(SUCCESS_COLOR).setTitle("🔓 Member Unbanned").addFields({ name: "User", value: `${ban.user.tag} (${ban.user.id})` }).setTimestamp()); } catch {} });
client.on("voiceStateUpdate", async (oldState, newState) => {
  try {
    ...
    else desc = `<@${newState.id}> moved from **${oldState.channel?.name}** to **${newState.channel?.name}**`;
