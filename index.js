...
const DOCS = [
  ...
  { name: "Luau Language Reference", value: "https://luau-lang.org/" },
  {
    ...
    value: "https://devforum.roblox.com/c/help-and-feedback/scripting-support/55",
  },
  { name: "Roblox Style Guide", value: "https://roblox.github.io/lua-style-guide/" },
];
...
function parseDuration(str) {
  if (!str) return null;
  const match = String(str).trim().toLowerCase().match(/^(\d+)\s*(s|m|h|d)?$/);
...
const COMMAND_LIST = [
  {
    ...
    items: [
      ...
      { name: "s!status", desc: "Show whether the bot and services are online." },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "s!debug", desc: "Format template for getting help with errors." },
    ],
  },
  {
    category: "Moderation",
    items: [
      ...
      { name: "s!warn <user> <reason>", desc: "Warn a user. Stored for later review." },
    ],
  },
  {
    category: "Partnership",
    items: [
      { name: "s!partner <invite> <info>", desc: "Post a partnership announcement with @here." },
    ],
  },
  {
    category: "Tickets",
    items: [
      { name: "s!ticketpanel", desc: "Post the ticket creation panel in this channel." },
      { name: "s!close", desc: "Close the current ticket channel." },
      { name: "s!addnote <text>", desc: "Add an internal staff note to the ticket." },
    ],
  },
  {
...
async function handleQueue(message) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(BRAND_COLOR)
    .setFooter({ text: `${active.length} active order${active.length === 1 ? "" : "s"}` });

  if (active.length === 0) {
    embed.setDescription(`The queue is empty. Use \`${PREFIX}ticket\` to request a commission.`);
  } else {
...
async function handleStatusOrder(message, args) {
  const id = args[0];
  if (!id) {
    await message.reply(`Usage: \`${PREFIX}statusorder <id>\` — example: \`${PREFIX}statusorder 3\``);
    return;
  }
...
async function handleBan(message, args) {
  ...
  try {
    ...
    await message.reply("Failed to ban that user. Check my role hierarchy and permissions.");
  }
}

async function handleKick(message, args) {
  if (!hasPerm(message.member, PermissionFlagsBits.KickMembers)) {
    await message.reply("You need the **Kick Members** permission to use this.");
    return;
  }
...
async function handleMute(message, args) {
  if (!hasPerm(message.member, PermissionFlagsBits.ModerateMembers)) {
    await message.reply("You need the **Timeout Members** permission to use this.");
    return;
  }
  ...
  const userId = parseUserId(args[0]);
  if (!userId || !args[1]) {
    await message.reply(`Usage: \`${PREFIX}mute <@user> <time> [reason]\` — e.g. \`${PREFIX}mute @user 10m spamming\``);
    return;
  }

  const ms = parseDuration(args[1]);
  if (!ms) {
    await message.reply("Invalid duration. Use formats like `30s`, `10m`, `2h`, `1d`.");
    return;
  }
...
async function handleWarn(message, args) {
  if (!hasPerm(message.member, PermissionFlagsBits.ModerateMembers)) {
    await message.reply("You need the **Timeout Members** permission to issue warnings.");
    return;
  }
...
async function handlePartner(message, args) {
  if (!isAdmin(message.member) && !hasPerm(message.member, PermissionFlagsBits.ManageGuild)) {
    await message.reply("You need the **Manage Server** permission to post partnerships.");
    return;
  }

  if (args.length < 2) {
    await message.reply(`Usage: \`${PREFIX}partner <invite> <info about the server>\``);
    return;
  }
  ...
  if (!/^https?:\/\/(discord\.gg|discord\.com\/invite|dsc\.gg)\//i.test(invite)) {
    await message.reply("Please provide a valid Discord invite link (e.g. https://discord.gg/abcd).");
    return;
  }
...
async function handleTicketPanel(message) {
  if (!isAdmin(message.member) && !hasPerm(message.member, PermissionFlagsBits.ManageChannels)) {
    await message.reply("You need the **Manage Channels** permission to post the panel.");
    return;
  }
...
async function handleAddNote(message, args) {
  ...
  if (!isAdmin(message.member) && !hasPerm(message.member, PermissionFlagsBits.ManageMessages)) {
    await message.reply("You need to be staff (Manage Messages) to add internal notes.");
    return;
  }
...
async function openTicketForUser(channel, member, formAnswers) {
  ...
  if (existing) {
    return { ok: false, error: `You already have an open ticket: <#${existing.id}>`, channel: existing };
  }
  ...
  try {
    ...
    return {
      ok: false,
      error: "I couldn't create your ticket. Make sure I have the **Manage Channels** permission.",
    };
  }
    ...
    if (interaction.isButton()) {
      if (interaction.customId === "ticket_open") {
        ...
        const descInput = new TextInputBuilder()
          ...
          .setPlaceholder("Describe what you need built — be as specific as you can.")
