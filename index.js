...
// ─────────────────────────────────────────────
//  AI Ticket Handler — OpenRouter with fallback
// ─────────────────────────────────────────────
const ticketConversations = new Map();
const ticketAIDisabled = new Set(); // channel IDs where ticket AI has been manually turned off
...
async function handleAntiRaidJoin(member) {
  ...
  if (tracker.locked && (settings.action === "kick" || settings.action === "ban")) {
    try {
      // DM the user before removing them
...
async function handleNick(message, args) {
  ...
  await respond(message, { embeds: [successEmbed("✏️ Nickname Updated").addFields({ name: "User", value: `<@${userId}>`, inline: true }, { name: "Nickname", value: nick || "*(reset)*", inline: true }).setTimestamp()] });
}
...
async function handleAI(message, args) {
  ...
  try {
    ...
    // Split into ≤1900-char plain-text chunks (safe for Discord message limit)
    const chunks = [];
    let rem = aiText;
...
async function openTicketForUser(channel, member, ticketType, formAnswers) {
  ...
  // AI greeting — non-blocking, won't crash ticket creation if it fails
