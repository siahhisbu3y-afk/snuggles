...
// ─── FFMPEG FIX — must be before @discordjs/voice loads ──────────────────────
try {
  process.env.FFMPEG_PATH = require("ffmpeg-static");
  console.log("✅ FFmpeg loaded from ffmpeg-static");
} catch {
  console.warn("⚠️  ffmpeg-static not found — run: npm install ffmpeg-static");
}
...
async function registerSlashCommands() {
  if (!CLIENT_ID || !TOKEN) {
    console.warn("⚠️  Skipping slash command registration — missing CLIENT_ID or TOKEN");
    return;
  }
  try {
...
// ═══════════════════════════════════════════════════════════════════════════
//  24/7 LOFI VC — FIXED: proper FFmpeg stream handling + error rate limiting
// ═══════════════════════════════════════════════════════════════════════════
...
let lofiRetry        = null;
let lofiErrorCount   = 0;
let lofiRunning      = false;

async function startLofiVC() {
  clearTimeout(lofiRetry);

  // Check FFmpeg is available before attempting to start
  const ffmpegPath = process.env.FFMPEG_PATH;
  if (!ffmpegPath) {
    console.error("[Lofi] ❌ FFmpeg not found — lofi disabled. Run: npm install ffmpeg-static");
    return;
  }
  ...
  try {
    ...
    if (!vc) { console.warn("[Lofi] VC channel not found:", LOFI_VC_CHANNEL_ID); return; }
    ...
    conn.on(VoiceConnectionStatus.Disconnected, () => {
      console.log("[Lofi] Disconnected — retrying in 10s");
      lofiRunning = false;
    ...
    conn.on("error", err => {
      ...
      lofiRunning = false;
      lofiRetry = setTimeout(startLofiVC, 15_000);
    });
    ...
    const playStream = async () => {
      ...
      try {
        // Use ffmpeg-based streaming via @discordjs/voice with proper stream type
        const { default: fetch: nodeFetch } = await import("node-fetch").catch(() => ({ default: global.fetch }));

        const resource = createAudioResource(track.url, {
          inputType: StreamType.Arbitrary, // forces FFmpeg transcoding
          inlineVolume: true,
        });
        resource.volume?.setVolume(0.35);
        player.play(resource);
        lofiErrorCount = 0; // reset on success
      } catch (err) {
        lofiErrorCount++;
        console.error("[Lofi] Stream error:", err.message);

        // Stop retrying if FFmpeg is genuinely missing
        if (err.message.toLowerCase().includes("ffmpeg") || err.message.toLowerCase().includes("avconv")) {
          console.error("[Lofi] ❌ FFmpeg not found! Fix: npm install ffmpeg-static");
          console.error("[Lofi] ❌ Then add at top of index.js: process.env.FFMPEG_PATH = require('ffmpeg-static')");
          lofiRunning = false;
          return; // Do NOT retry — it will never work without FFmpeg
        }

        // For other errors, back off with increasing delay (max 30s)
        if (lofiErrorCount > 10) {
          console.warn("[Lofi] Too many errors — pausing for 60s before retry");
          lofiErrorCount = 0;
          setTimeout(playStream, 60_000);
        } else {
          setTimeout(playStream, Math.min(5000 * lofiErrorCount, 30_000));
        }
      }
    };

    player.on(AudioPlayerStatus.Idle, () => {
      if (lofiRunning) setTimeout(playStream, 2000);
    });
    ...
    player.on("error", err => {
      ...
      if (err.message.toLowerCase().includes("ffmpeg") || err.message.toLowerCase().includes("avconv")) {
        console.error("[Lofi] ❌ FFmpeg missing — stopping lofi retries. Run: npm install ffmpeg-static");
        lofiRunning = false;
        return;
      }

      if (lofiErrorCount > 10) {
        console.warn("[Lofi] Too many player errors — pausing 60s");
        lofiErrorCount = 0;
        setTimeout(playStream, 60_000);
      } else {
        setTimeout(playStream, Math.min(5000 * lofiErrorCount, 30_000));
    ...
    if (err.message?.includes("Cannot find module")) {
      console.warn("[Lofi] @discordjs/voice not installed — run: npm install @discordjs/voice");
    } else {
      console.warn("[Lofi] Failed to start:", err.message);
    }
  }
}
...
client.once("clientReady", async () => {
  ...
  console.log(`🎵  Music: play-dl`);
  console.log(`🔧  FFmpeg: ${process.env.FFMPEG_PATH ? "✅ Ready" : "❌ Missing (run: npm install ffmpeg-static)"}`);
