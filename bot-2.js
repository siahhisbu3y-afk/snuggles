...
const ERR_CHANNEL_FALLBACK = "1497080858048462948";

const ENV = {
  HOME:   process.env.HOME_GUILD_ID?.trim()            || "",
  ERR:    process.env.ERROR_CHANNEL_ID?.trim()         || ERR_CHANNEL_FALLBACK,
...
const DAILY = [
  ...
  { text: "🎁 **AI Chat Token** — ask me anything with `/ai`!",                       coins: 55  },
];

const RULES = [
  "**1. Be Respectful** — Treat everyone with kindness. No harassment, hate speech, discrimination, or personal attacks.",
  "**2. No Spam or Flooding** — Don't spam messages, emojis, caps, or repeat the same thing in multiple channels.",
  "**3. Use Channels Properly** — Post content in the correct channels. Keep off-topic conversations to the appropriate places.",
  "**4. No NSFW Content** — Keep the server SFW (Safe For Work). No explicit, adult, or graphic content unless the server is 18+.",
  "**5. No Advertising / Self-Promo** — Do not advertise other servers, social media, or products without permission from staff.",
  "**6. No Toxicity / Drama** — Avoid starting arguments, drama, or bringing negativity into the server.",
  "**7. Follow Discord's Terms of Service** — Any behavior that violates Discord's rules (e.g. threats, doxxing, scams) will result in an immediate ban.",
  "**8. No Mini-Modding** — Let the staff handle rule enforcement. Don't argue with or try to police other members.",
  "**9. Keep Conversations Civil** — Even during disagreements, stay respectful and avoid excessive swearing or hostility.",
  "**10. Have Fun & Be Active!** — This is a community — participate, make friends, and enjoy your time here!",
];
...
function defaultData() {
  return {
    ...
    stats: {
      ...
      reviewsSubmitted: 0, imagesGenerated: 0,
    },
...
const CDS = {
  ...
  coinflip: 3000, roll: 3000, rps: 3000, serverinfo: 5000,
...
async function errLog(err, ctx = "") {
  try {
    const errChId = ENV.ERR;
    if (!errChId) return;
    let ch = null;
    if (ENV.HOME) {
      const g = client.guilds.cache.get(ENV.HOME);
      if (g) ch = await g.channels.fetch(errChId).catch(() => null);
    }
    if (!ch) {
      for (const guild of client.guilds.cache.values()) {
        ch = await guild.channels.fetch(errChId).catch(() => null);
        if (ch) break;
      }
    }
    if (!ch?.isTextBased()) return;
    ch.send({ embeds: [E.er("🚨 Bot Error").addFields({ name: "Context", value: ctx || "—" }, { name: "Error", value: `\`\`\`${String(err?.message || err).slice(0, 900)}\`\`\`` }).setTimestamp()] });
  } catch {}
}

// ── MOD DM HELPER ─────────────────────────────────────────────────────────────
async function sendModDM(target, guildName, action, reason, moderatorTag, extra = {}) {
  const actionEmoji = {
    ban: "🔨", kick: "👢", mute: "🔇", warn: "⚠️", softban: "🧹",
  }[action] || "⚠️";
  const actionLabel = {
    ban: "banned", kick: "kicked", mute: "timed out", warn: "warned", softban: "softbanned",
  }[action] || action;

  const emb = new EmbedBuilder()
    .setColor(action === "warn" ? C.warn : C.error)
    .setTitle(`${actionEmoji} You have been **${actionLabel}**`)
    .setDescription(`You have been **${actionLabel}** from **${guildName}**.`)
    .addFields(
  ...
  if (extra.duration) emb.addFields({ name: "Duration", value: extra.duration, inline: true });
...
function buildHelp(page = 1) {
  ...
  const pages = [
    E.b("📖 Help — General & AI").setAuthor(auth).setDescription(`> Prefix: \`s!\`  ·  Slash: \`/\`  — both work!\n${DIV}`)
      .addFields(
        { name: "🌐 General",       value: "`help [page]`  `info`  `ping`  `status`  `rules`  `terms`\n`uptime`  `invite`  `setup`  `stats`" },
        { name: "🤖 AI",            value: "`ai <message>` — Chat with Snuggles AI\n`ai-reset` — Wipe conversation history\n`aioff` / `aion` — Toggle AI in ticket *(staff)*" },
...
async function callOpenRouter(messages, system, opts = {}) {
  if (!OPENROUTER_KEY) return { text: null, model: null, error: "No OpenRouter API key configured." };
  try {
    const msgs = [
    ...
    const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        "HTTP-Referer":  "https://snugglesscripting.com",
        "X-Title":       "Snuggles Scripting Bot",
      },
      body: JSON.stringify({
    ...
    if (!res.ok) {
      ...
      console.error(`[OpenRouter] ${res.status}:`, msg);
      return { text: null, model: null, error: msg };
    }
    const j = await res.json();
    const text = j.choices?.[0]?.message?.content?.trim() || null;
    const modelName = j.model || "OpenRouter AI";
    if (!text) return { text: null, model: null, error: "No response generated." };
...
async function callTicketAI(chId, msg, type, ctx) {
  ...
  const r = await callOpenRouter(h, sys, { maxTokens: 900 });
...
function isScamMessage(content) {
  ...
  for (const pattern of SCAM_PATTERNS) { if (pattern.test(content)) return true; }
  for (const keyword of SCAM_KEYWORDS)  { if (lower.includes(keyword)) return true; }
  return false;
}
...
async function handleScamDetection(message) {
  ...
  const uid = message.author.id, gid = message.guild.id, key = `${gid}:${uid}`;
  ...
  if (count >= 3) {
    if (message.member?.bannable) { await member.ban({ reason: "Repeated scam messages — auto-banned." }).catch(() => {}); actionTaken = "Banned"; }
  } else if (count >= 2) {
    if (message.member?.moderatable) { await message.member.timeout(10 * 60 * 1000, "Scam/phishing message.").catch(() => {}); actionTaken = "Timed out (10 minutes)"; }
  }
...
// ── CTX BUILDER ───────────────────────────────────────────────────────────────
function buildCtx(slash, thing) {
  if (slash) {
    const i = thing;
    return {
      slash: true, i, channel: i.channel,
    ...
    return {
      ...
      grm: _  => msg.mentions?.roles?.first()  || null,
      ga:  i  => parts[i] || null,
      gas: () => parts,
      gsub: () => parts[0] || null,
      reply: (p, eph) => { if (typeof p === "string") return msg.reply(p).catch(() => {}); return msg.reply(p).catch(() => {}); },
    };
  }
}
...
async function exec(cmd, ctx) {
  ...
  switch (cmd) {
    ...
    case "info": {
      ...
      return ctx.reply({ embeds: [E.b(`🧸 ${NAME}`).setThumbnail(client.user?.displayAvatarURL()).addFields({ name: "🤖 Tag", value: client.user?.tag || "—", inline: true }, { name: "📦 Version", value: `v${VER}`, inline: true }, { name: "👑 Owner", value: OWNER, inline: true }, { name: "🌐 Servers", value: `${client.guilds.cache.size}`, inline: true }, { name: "⏱️ Uptime", value: `${h}h ${m}m ${s2}s`, inline: true }, { name: "📋 Orders", value: `${data.orders.length}`, inline: true }, { name: "🤖 AI", value: OPENROUTER_KEY ? "✅ OpenRouter AI" : "❌ Disabled", inline: true }, { name: "🟣 Verified", value: `${Object.keys(data.robloxVerified).length} users`, inline: true }).setFooter({ text: FT }).setTimestamp()] });
    }
    case "ping": {
      if (ctx.slash) {
    ...
    case "status": {
      ...
      return ctx.reply({ embeds: [E.ok("🟢 All Systems Operational").addFields({ name: "🤖 Bot", value: `🟢 Online · ${Math.round(client.ws.ping)}ms`, inline: true }, { name: "📋 Active Orders", value: `${data.orders.filter(o => o.status !== "completed" && o.status !== "cancelled").length}`, inline: true }, { name: "🤖 OpenRouter AI", value: OPENROUTER_KEY ? "🟢 Ready" : "🔴 No Key", inline: true }, { name: "🟣 Verified", value: `${Object.keys(data.robloxVerified).length} users`, inline: true }, { name: "🛡️ Anti-Raid", value: ar?.enabled ? "🟢 Active" : "🔴 Off", inline: true }, { name: "☢️ Anti-Nuke", value: an?.enabled ? "🟢 Active" : "🔴 Off", inline: true }, { name: "🌐 Servers", value: `${client.guilds.cache.size}`, inline: true }).setFooter({ text: FT }).setTimestamp()] });
    }

    case "rules": {
      const emb = E.b("📜 Server Rules")
        .setDescription("Please read and follow all rules to keep our community a great place for everyone.")
        .setFooter({ text: `${RULES.length} Rules · ${FT}` })
        .setTimestamp();
      RULES.forEach(rule => {
        const match = rule.match(/^\*\*(\d+\.\s[^*]+)\*\*/);
        const title = match ? match[0] : rule.split(" — ")[0];
        const desc  = rule.includes(" — ") ? rule.split(" — ").slice(1).join(" — ") : rule;
        emb.addFields({ name: title, value: desc, inline: false });
      });
    ...
    case "terms": {
      return ctx.reply({ embeds: [
        E.b("📜 Terms of Service & Privacy Policy")
          .setDescription("Please review our legal documents below.")
          .addFields(
            { name: "📜 Terms of Service", value: "[Read here](https://docs.google.com/document/d/13dYCdrmj9mU9jWmEYONExBDnfcwYlQfxR6UJdZRsP28/edit?usp=drivesdk)" },
            { name: "🔒 Privacy Policy",   value: "[Read here](https://docs.google.com/document/d/16ppiOOtWtPkarJVjmxbsHBlObJFQg8IIrnMOaFhDcXk/edit?usp=drivesdk)" },
          )
          .setFooter({ text: FT })
          .setTimestamp()
      ]});
    ...
    // ── AI ────────────────────────────────────────────────────────────────────
    case "ai": {
      if (!OPENROUTER_KEY) return ctx.reply({ embeds: [E.er("🤖 AI Disabled").setDescription("The AI is not configured. Please contact server staff.")] }, true);
      ...
      const { text, model, error } = await callOpenRouter(hist, SYS.chat, { maxTokens: 1000, temp: 0.75 });
      if (!text) return ctx.reply({ embeds: [E.w("🤖 AI Unavailable").setDescription("The AI couldn't generate a response right now. Please try again in a moment.").setFooter({ text: FT })] });
    ...
    case "announce": {
      ...
      const emb = new EmbedBuilder().setColor(C.brand).setTitle("📢 Announcement").setDescription(message).setFooter({ text: `Announced by ${user.tag} · ${guild.name}` }).setTimestamp();
    ...
    case "remindme": {
      ...
      if (!timeStr || !msg2) return ctx.reply({ embeds: [E.w("Usage: `s!remindme <time> <message>`")] }, true);
    ...
    case "calc": {
      ...
      } catch { return ctx.reply({ embeds: [E.er("Invalid expression.")] }, true); }
    }
    ...
    case "invites": {
      ...
      try {
        ...
        const activeInvites  = discordInvites.filter(i => i.inviterId === target.id);
        const liveUses       = activeInvites.reduce((sum, i) => sum + (i.uses || 0), 0);
        const trackedCount   = data.invites[guild.id]?.[target.id] || 0;
        const totalInvites   = Math.max(liveUses, trackedCount);
        return ctx.reply({ embeds: [
          E.i(`📨 Invites — ${target.username}`)
            .setThumbnail(target.displayAvatarURL())
            .addFields(
              { name: "📨 Total Invites",  value: `**${totalInvites}**`,         inline: true },
              { name: "🔗 Active Links",   value: `${activeInvites.size}`,        inline: true },
            )
            .setFooter({ text: FT }).setTimestamp()
        ]});
      } catch {
    ...
    case "inviteleaderboard": {
      ...
      try {
        const discordInvites = await guild.invites.fetch();
        // Build live uses map per inviter
        const liveMap = {};
        for (const inv of discordInvites.values()) {
          if (!inv.inviterId) continue;
          liveMap[inv.inviterId] = (liveMap[inv.inviterId] || 0) + (inv.uses || 0);
        }
        // Merge with tracked data (take highest value)
        const trackedMap = data.invites[guild.id] || {};
        const merged = {};
        const allIds = new Set([...Object.keys(liveMap), ...Object.keys(trackedMap)]);
        for (const id of allIds) {
          merged[id] = Math.max(liveMap[id] || 0, trackedMap[id] || 0);
        }
        const top = Object.entries(merged).filter(([, c]) => c > 0).sort((a, b) => b[1] - a[1]).slice(0, 10);
        ...
        const lines  = await Promise.all(top.map(async ([id, count], i) => {
          ...
          return `${medals[i] || `**${i + 1}.**`} ${name} — **${count}** invite${count !== 1 ? "s" : ""}`;
        }));
        return ctx.reply({ embeds: [E.g("📨 Invite Leaderboard").setDescription(lines.join("\n")).setFooter({ text: `Live data from Discord · ${FT}` }).setTimestamp()] });
      } catch {
    ...
    case "checkverify": {
      ...
      if (pending.code !== code) return ctx.reply({ embeds: [E.er("Wrong Code").setDescription("That code doesn't match.")] }, true);
    ...
    case "whois": {
      const target = ctx.gu("user") || user;
      const roblox = data.robloxVerified[target.id];
      if (!roblox) return ctx.reply({ embeds: [E.w("Not Verified").setDescription(`**${target.username}** has not linked a Roblox account.`)] }, true);
      if (ctx.slash) await ctx.i.deferReply().catch(() => {});
    ...
    case "serverscan": {
      ...
      if (ctx.slash) await ctx.i.deferReply().catch(() => {});
      let members2;
      try { members2 = await guild.members.fetch(); } catch { return ctx.reply({ embeds: [E.er("Failed to fetch members.")] }); }
      ...
      return ctx.reply({ embeds: [
        E.mk(riskScore >= 40 ? C.error : riskScore >= 15 ? C.warn : C.success, "🔍 Server Security Scan")
          ...
          .addFields(
            ...
            { name: "💡 Recommendation",     value: riskScore >= 40 ? "⚠️ High risk detected! Enable `/antiraid` immediately." : riskScore >= 15 ? "Monitor new joins closely." : "Server looks healthy!" },
          )
    ...
    case "roleinfo": {
      ...
      return ctx.reply({ embeds: [E.mk(role.color || C.brand, `🎭 Role: ${role.name}`).addFields({ name: "🆔 Role ID", value: role.id, inline: true }, { name: "🎨 Color", value: `#${role.color.toString(16).padStart(6, "0").toUpperCase()}`, inline: true }, { name: "📊 Position", value: `${role.position}`, inline: true }, { name: "👥 Members", value: `${membersWithRole.size}`, inline: true }, { name: "🏆 Hoisted", value: role.hoist ? "Yes" : "No", inline: true }, { name: "📢 Mentionable", value: role.mentionable ? "Yes" : "No", inline: true }, { name: "🤖 Managed", value: role.managed ? "Yes (Bot/Integration)" : "No", inline: true }, { name: "📅 Created", value: `<t:${Math.floor(role.createdTimestamp / 1000)}:D>`, inline: true }, { name: "🔑 Key Permissions", value: permsNames }).setFooter({ text: FT }).setTimestamp()] });
    }

    case "channelinfo": {
      if (!guild) return ctx.reply({ embeds: [E.er("Server only.")] }, true);
      const ch2 = (ctx.slash ? ctx.i.options.getChannel("channel") : null) || ctx.channel;
      return ctx.reply({ embeds: [E.i(`📡 Channel: #${ch2.name}`).addFields({ name: "🆔 Channel ID", value: ch2.id, inline: true }, { name: "📂 Type", value: ch2.type != null ? ChannelType[ch2.type] || `${ch2.type}` : "Unknown", inline: true }, { name: "📁 Category", value: ch2.parent?.name || "None", inline: true }, { name: "📌 Position", value: `${ch2.rawPosition ?? "—"}`, inline: true }, { name: "🔞 NSFW", value: ch2.nsfw ? "Yes" : "No", inline: true }, { name: "🐢 Slowmode", value: ch2.rateLimitPerUser ? `${ch2.rateLimitPerUser}s` : "Off", inline: true }, { name: "📅 Created", value: `<t:${Math.floor(ch2.createdTimestamp / 1000)}:D>`, inline: true }, ...(ch2.topic ? [{ name: "📝 Topic", value: ch2.topic.slice(0, 200) }] : [])).setFooter({ text: FT }).setTimestamp()] });
    }
    ...
    case "membercount": {
      ...
      return ctx.reply({ embeds: [E.i(`👥 Member Count — ${guild.name}`).setThumbnail(guild.iconURL()).addFields({ name: "👥 Total", value: `${guild.memberCount}`, inline: true }, { name: "👤 Humans", value: `${humans.size}`, inline: true }, { name: "🤖 Bots", value: `${bots3.size}`, inline: true }, { name: "🟢 Online", value: `${online.size}`, inline: true }, { name: "💎 Boosting", value: `${boosting.size}`, inline: true }, { name: "🏅 Boost Level", value: `${guild.premiumTier}`, inline: true }).setFooter({ text: FT }).setTimestamp()] });
    }

    case "snipe": {
      if (!guild) return ctx.reply({ embeds: [E.er("Server only.")] }, true);
      const sniped = snipeCache.get(ctx.channel.id);
      if (!sniped) return ctx.reply({ embeds: [E.w("👻 Nothing to snipe.")] });
      return ctx.reply({ embeds: [E.mk(C.note, "👻 Sniped Message").setDescription(`"${sniped.content.slice(0, 2000)}"` || "*[no text content]*").setAuthor({ name: sniped.author, iconURL: sniped.authorAvatar || undefined }).addFields({ name: "🕐 Deleted", value: `<t:${Math.floor(sniped.timestamp / 1000)}:R>`, inline: true }).setFooter({ text: FT }).setTimestamp()] });
    }

    case "editsnipe": {
      if (!guild) return ctx.reply({ embeds: [E.er("Server only.")] }, true);
      const eSniped = editSnipeCache.get(ctx.channel.id);
      if (!eSniped) return ctx.reply({ embeds: [E.w("✏️ Nothing to edit-snipe.")] });
      return ctx.reply({ embeds: [E.mk(C.note, "✏️ Edit Sniped Message").setAuthor({ name: eSniped.author, iconURL: eSniped.authorAvatar || undefined }).addFields({ name: "📝 Before", value: (eSniped.before || "*empty*").slice(0, 1000) }, { name: "✏️ After", value: (eSniped.after || "*empty*").slice(0, 1000) }, { name: "🕐 Edited", value: `<t:${Math.floor(eSniped.timestamp / 1000)}:R>`, inline: true }).setFooter({ text: FT }).setTimestamp()] });
    }
    ...
    case "nuke": {
      ...
      const ch3 = ctx.channel, pos3 = ch3.rawPosition, parent3 = ch3.parentId, topic3 = ch3.topic;
      ...
      try {
        ...
        for (const ow of overw3) { await newCh.permissionOverwrites.create(ow.id, {}).catch(() => {}); }
      ...
      } catch { return ctx.reply({ embeds: [E.er("Nuke failed — check my **Manage Channels** permission.")] }, true); }
      break;
    }
    ...
    case "listbans": {
      ...
      return ctx.reply({ embeds: [E.mk(C.error, `🔨 Ban List — ${bans.size} banned`).setDescription(list2).setFooter({ text: `Showing ${Math.min(bans.size, 20)} of ${bans.size} · ${FT}` }).setTimestamp()] });
    }
    ...
    case "steal": {
      ...
      let imageUrl;
      if (emojiMatch) { const animated = emojiStr.startsWith("<a:"); imageUrl = `https://cdn.discordapp.com/emojis/${emojiMatch[1]}.${animated ? "gif" : "png"}`; }
      else if (/^https?:\/\//.test(emojiStr)) { imageUrl = emojiStr; }
      else return ctx.reply({ embeds: [E.w("Provide a custom emoji or direct image URL.")] }, true);
      try {
        ...
        return ctx.reply({ embeds: [E.ok("😎 Emoji Stolen!").setDescription(`${created} \`:${created.name}:\` has been added!`).setFooter({ text: FT }).setTimestamp()] });
      } catch { return ctx.reply({ embeds: [E.er("Failed to steal emoji — check permissions or try a different image.")] }, true); }
    }
    ...
    case "firstmessage": {
      ...
      try {
        ...
        if (!msg2) return ctx.reply({ embeds: [E.w("No messages found.")] }, true);
        return ctx.reply({ embeds: [E.i("📌 First Message").setDescription(`[Jump to message](${msg2.url})`).addFields({ name: "📝 Content", value: (msg2.content || "*[no text]*").slice(0, 500) }, { name: "👤 Author", value: `<@${msg2.author.id}>`, inline: true }, { name: "📅 Sent", value: `<t:${Math.floor(msg2.createdTimestamp / 1000)}:f>`, inline: true }).setFooter({ text: FT }).setTimestamp()] });
    ...
    case "botperms": {
      ...
      const chPerms = ctx.channel.permissionsFor(botMember), gldPerms = botMember.permissions;
      const critPerms = [
      ...
      return ctx.reply({ embeds: [E.i("🔑 Bot Permissions").setDescription(critPerms.map(([n, v]) => `${v ? "✅" : "❌"} \`${n}\``).join("\n")).addFields({ name: "📢 Channel", value: `<#${ctx.channel.id}>`, inline: true }).setFooter({ text: FT }).setTimestamp()] });
    }
    ...
    case "encode": {
      ...
      try {
        const result2 = mode2 === "decode" ? Buffer.from(encText, "base64").toString("utf8") : Buffer.from(encText, "utf8").toString("base64");
        return ctx.reply({ embeds: [E.i(`🔒 Base64 ${mode2 === "decode" ? "Decoded" : "Encoded"}`).addFields({ name: "Input", value: `\`\`\`${encText.slice(0, 500)}\`\`\`` }, { name: "Output", value: `\`\`\`${result2.slice(0, 1000)}\`\`\`` }).setFooter({ text: FT })] });
    ...
    case "afk": {
      ...
      if (!reason4) {
        if (afkUsers.has(user.id)) { afkUsers.delete(user.id); return ctx.reply({ embeds: [E.ok("✅ Welcome back!").setDescription("Your AFK status has been removed.").setFooter({ text: FT })] }); }
    ...
    case "boosters": {
      ...
      return ctx.reply({ embeds: [E.mk(0xFF73FA, `💎 Server Boosters — ${boostM.size}`).setDescription(lines2.slice(0, 4000)).addFields({ name: "🏅 Boost Level", value: `${guild.premiumTier}`, inline: true }, { name: "📊 Boosts", value: `${guild.premiumSubscriptionCount || 0}`, inline: true }).setFooter({ text: FT }).setTimestamp()] });
    }
    ...
    case "perms": {
      ...
      chunks.forEach((chunk, i) => { emb5.addFields({ name: i === 0 ? "✅ Has Permissions" : "\u200B", value: chunk.map(p => `\`${p.toLowerCase().replace(/_/g, " ")}\``).join(", ") }); });
    ...
    case "define": {
      ...
      try {
        ...
        const meaning = entry.meanings?.[0], def5 = meaning?.definitions?.[0];
        const phonetic = entry.phonetic || entry.phonetics?.find(p => p.text)?.text || "";
        const emb6 = E.b(`📖 ${entry.word}`).setDescription(phonetic ? `*${phonetic}*` : "").addFields({ name: `📝 ${meaning?.partOfSpeech || "Definition"}`, value: def5?.definition || "No definition found." });
      ...
      } catch { return ctx.reply({ embeds: [E.er("Dictionary API unavailable.")] }, true); }
    }
    ...
    case "antinuke": {
      ...
      if (sub === "enable")  { s.enabled = true;  saveData(); return ctx.reply({ embeds: [E.ok("☢️ Anti-Nuke Enabled").setFooter({ text: FT })] }); }
      if (sub === "disable") { s.enabled = false; saveData(); return ctx.reply({ embeds: [E.w("☢️ Anti-Nuke Disabled").setFooter({ text: FT })] }); }
      if (sub === "status")  { return ctx.reply({ embeds: [E.mk(s.enabled ? C.success : C.warn, `☢️ Anti-Nuke — ${s.enabled ? "🟢 Active" : "🔴 Disabled"}`).addFields({ name: "⏱️ Window", value: `${s.window / 1000}s`, inline: true }, { name: "🎯 Action", value: s.action.toUpperCase(), inline: true }, { name: "📨 Ban Threshold", value: `${s.banThreshold}`, inline: true }, { name: "💬 Channel Delete", value: `${s.channelDeleteThreshold}`, inline: true }, { name: "🎭 Role Delete", value: `${s.roleDeleteThreshold}`, inline: true }, { name: "👢 Kick", value: `${s.kickThreshold}`, inline: true }, { name: "👥 Trusted Users", value: `${s.trustedUsers?.length || 0}`, inline: true }, { name: "📢 Notify", value: s.notifyChannel ? `<#${s.notifyChannel}>` : "Not set", inline: true }).setFooter({ text: FT }).setTimestamp()] }); }
      if (sub === "config")  { const banth = ctx.gi("banth"), channelth = ctx.gi("channelth"), roleth = ctx.gi("roleth"), kickth = ctx.gi("kickth"), action = ctx.gs("action"), notify = ctx.gc("notify"); if (banth != null) s.banThreshold = banth; if (channelth != null) s.channelDeleteThreshold = channelth; if (roleth != null) s.roleDeleteThreshold = roleth; if (kickth != null) s.kickThreshold = kickth; if (action != null) s.action = action; if (notify) s.notifyChannel = notify.id; saveData(); return ctx.reply({ embeds: [E.ok("☢️ Anti-Nuke Updated").setFooter({ text: FT })] }); }
      if (sub === "trust")   { const target = ctx.gu("user"); if (!target) return ctx.reply({ embeds: [E.w("Provide a user.")] }, true); if (!s.trustedUsers) s.trustedUsers = []; if (!s.trustedUsers.includes(target.id)) s.trustedUsers.push(target.id); saveData(); return ctx.reply({ embeds: [E.ok("✅ User Trusted").setDescription(`<@${target.id}> is exempt from anti-nuke.`).setFooter({ text: FT })] }); }
    ...
    case "ban": {
      ...
      if (tm && !tm.bannable) return ctx.reply({ embeds: [E.er("I cannot ban this user.")] }, true);
      ...
      saveData();
      await sendModDM(target, guild.name, "ban", reason, user.tag);
    ...
    case "softban": {
      ...
      await sendModDM(target, guild.name, "softban", reason, user.tag);
    ...
    case "kick": {
      ...
      await sendModDM(target, guild.name, "kick", reason, user.tag);
    ...
    case "mute": {
      ...
      await sendModDM(target, guild.name, "mute", reason, user.tag, { duration: fmtDur(ms) });
    ...
    case "warn": {
      ...
      saveData();
      await sendModDM(target, guild.name, "warn", reason, user.tag);
    ...
    case "purge": {
      ...
      return ctx.reply({ embeds: [E.ok("🗑️ Messages Purged").addFields({ name: "Deleted", value: `${deleted?.size || 0} messages`, inline: true })], flags: MessageFlags.Ephemeral });
    }
    case "lock": {
    ...
    case "poll": {
      ...
      const pollEmb = E.b(`📊 ${question}`).setDescription(opts.map((o, i) => `${nums[i] || `${i+1}.`} ${o}`).join("\n")).setFooter({ text: `${user.username} · ${FT}` }).setTimestamp();
      const msg2 = await ctx.channel.send({ embeds: [pollEmb] });
      for (let i = 0; i < Math.min(opts.length, nums.length); i++) { await msg2.react(nums[i]).catch(() => {}); }
      if (ctx.slash) await ctx.i.reply({ content: "✅ Poll posted!", flags: MessageFlags.Ephemeral }).catch(() => {});
      break;
    }
    case "giveaway": {
      if (!guild) return ctx.reply({ embeds: [E.er("Server only.")] }, true);
      ...
      const prize  = ctx.gs("prize")    || ctx.gas().slice(1).join(" ") || "Prize";
      const winners = ctx.gi("winners") || 1;
      const ms = parseDur(durStr);
      if (!ms) return ctx.reply({ embeds: [E.er("Invalid duration.")] }, true);
      const endAt = Date.now() + ms;
      const gaEmb = E.g("🎉 Giveaway!").setDescription(`React with 🎉 to enter!\n\n**Prize:** ${prize}\n**Winners:** ${winners}\n**Ends:** <t:${Math.floor(endAt / 1000)}:R>`).setFooter({ text: `${FT} · Hosted by ${user.username}` }).setTimestamp();
      const gMsg = await ctx.channel.send({ embeds: [gaEmb] });
      await gMsg.react("🎉").catch(() => {});
      data.giveaways[gMsg.id] = { guildId: guild.id, channelId: ctx.channel.id, prize, winners, endAt, ended: false, hostId: user.id };
      saveData();
    ...
    case "addorder": {
      ...
      const order = { id: data.nextOrderId++, userId: target.id, details, status: "not_started", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), note: "" };
      data.orders.push(order);
      data.stats.ordersCreated = (data.stats.ordersCreated || 0) + 1;
      saveData();
      await updateOrderBoard();
    ...
    case "updateorder": {
      if (!isStaff(member)) return ctx.reply({ embeds: [E.er("Staff only.")] }, true);
      const id = ctx.gi("id") || parseInt(ctx.ga(0), 10);
      const o  = findOrder(id); if (!o) return ctx.reply({ embeds: [E.er("Order not found.")] }, true);
      const status = ctx.gs("status") || ctx.ga(1);
      const note   = ctx.gs("note")   || ctx.gas().slice(2).join(" ");
      ...
      await updateOrderBoard();
      const si = gOS(o.status);
      return ctx.reply({ embeds: [E.ok(`🔄 Order #${o.id} Updated`).addFields({ name: "📊 Status", value: `${si.emoji} ${si.label}`, inline: true }, ...(note ? [{ name: "📋 Note", value: note, inline: true }] : [])).setFooter({ text: FT }).setTimestamp()] });
    }
    case "complete": {
      if (!isStaff(member)) return ctx.reply({ embeds: [E.er("Staff only.")] }, true);
      const id = ctx.gi("id") || parseInt(ctx.ga(0), 10);
      ...
      saveData();
      await updateOrderBoard();
      return ctx.reply({ embeds: [E.ok(`✅ Order #${o.id} Completed`).addFields({ name: "👤 Customer", value: `<@${o.userId}>`, inline: true }).setFooter({ text: FT }).setTimestamp()] });
    }
    case "blacklist": {
      if (!isAdmin(member)) return ctx.reply({ embeds: [E.er("Administrator only.")] }, true);
      const target = ctx.gu("user"); if (!target) return ctx.reply({ embeds: [E.w("Provide a user.")] }, true);
      const idx = data.blacklist.indexOf(target.id);
      if (idx !== -1) { data.blacklist.splice(idx, 1); saveData(); return ctx.reply({ embeds: [E.ok("✅ Removed from Blacklist").setDescription(`<@${target.id}> can now use the bot.`).setFooter({ text: FT })] }); }
      data.blacklist.push(target.id); saveData();
      return ctx.reply({ embeds: [E.er("🚫 Added to Blacklist").setDescription(`<@${target.id}> can no longer use the bot.`).setFooter({ text: FT })] });
    }
    case "setlog": {
      if (!isAdmin(member)) return ctx.reply({ embeds: [E.er("Administrator only.")] }, true);
      const ch = ctx.gc("channel") || ctx.channel;
      data.modLogChannels[guild.id] = ch.id;
      saveData();
      return ctx.reply({ embeds: [E.ok("✅ Mod Log Channel Set").setDescription(`Mod logs will be sent to <#${ch.id}>`).setFooter({ text: FT })] });
    }
    case "setreviews": {
      if (!isAdmin(member)) return ctx.reply({ embeds: [E.er("Administrator only.")] }, true);
      const ch = ctx.gc("channel");
      if (!ch) return ctx.reply({ embeds: [E.w("Provide a channel.")] }, true);
      gS(guild.id).reviewsChannelId = ch.id;
      saveData();
      return ctx.reply({ embeds: [E.ok("✅ Reviews Channel Set").setDescription(`Reviews will post in <#${ch.id}>`).setFooter({ text: FT })] });
    }
    case "settranscripts": {
      if (!isAdmin(member)) return ctx.reply({ embeds: [E.er("Administrator only.")] }, true);
      const ch = ctx.gc("channel");
      if (!ch) return ctx.reply({ embeds: [E.w("Provide a channel.")] }, true);
      gS(guild.id).transcriptsChannelId = ch.id;
      saveData();
      return ctx.reply({ embeds: [E.ok("✅ Transcripts Channel Set").setDescription(`Transcripts will post in <#${ch.id}>`).setFooter({ text: FT })] });
    }
    case "setverifyrole": {
      if (!isAdmin(member)) return ctx.reply({ embeds: [E.er("Administrator only.")] }, true);
      const role = ctx.gr("role"); if (!role) return ctx.reply({ embeds: [E.w("Provide a role.")] }, true);
      gS(guild.id).verifyRoleId = role.id;
      saveData();
      return ctx.reply({ embeds: [E.ok("✅ Verify Role Set").setDescription(`<@&${role.id}> will be given on verification.`).setFooter({ text: FT })] });
    }
    case "setlevelupchannel": {
      if (!isAdmin(member)) return ctx.reply({ embeds: [E.er("Administrator only.")] }, true);
      const ch = ctx.gc("channel"); if (!ch) return ctx.reply({ embeds: [E.w("Provide a channel.")] }, true);
      gS(guild.id).levelUpChannelId = ch.id;
      saveData();
      return ctx.reply({ embeds: [E.ok("✅ Level-Up Channel Set").setDescription(`Level-up messages will go to <#${ch.id}>`).setFooter({ text: FT })] });
    }
    case "ticketpanel": {
      if (!guild) return ctx.reply({ embeds: [E.er("Server only.")] }, true);
      if (!isAdmin(member)) return ctx.reply({ embeds: [E.er("Administrator only.")] }, true);
      ...
      return ctx.reply({ embeds: [E.ok("✅ Ticket panel posted!").setFooter({ text: FT })] }, true);
    }
    case "close": {
      if (!guild) return ctx.reply({ embeds: [E.er("Server only.")] }, true);
      if (!ctx.channel?.name?.startsWith("ticket-")) return ctx.reply({ embeds: [E.w("Only works in ticket channels.")] }, true);
      if (!isStaff(member)) return ctx.reply({ embeds: [E.er("Staff only.")] }, true);
      const reason = ctx.gs("reason") || ctx.gas().join(" ") || "No reason";
    ...
    case "addnote": {
      ...
      let targetId = null;
      for (const [id, o] of ctx.channel.permissionOverwrites.cache) { if (o.type === 1) { targetId = id; break; } }
      if (!targetId) return ctx.reply({ embeds: [E.er("Could not identify ticket owner.")] }, true);
      if (!data.warns[targetId]) data.warns[targetId] = [];
      data.warns[targetId].push({ id: data.nextWarnId++, type: "note", reason: text, modId: user.id, at: new Date().toISOString() });
      saveData();
      return ctx.reply({ embeds: [E.ok("📝 Note Added").setDescription(text).setFooter({ text: FT }).setTimestamp()] });
    }
    case "givecoins": {
      if (!isAdmin(member)) return ctx.reply({ embeds: [E.er("Administrator only.")] }, true);
      const target = ctx.gu("user"); if (!target) return ctx.reply({ embeds: [E.w("Provide a user.")] }, true);
      const amount = ctx.gi("amount") || parseInt(ctx.ga(1), 10) || 0;
      if (amount < 1) return ctx.reply({ embeds: [E.w("Amount must be at least 1.")] }, true);
      gE(target.id).coins = (gE(target.id).coins || 0) + amount;
      saveData();
      return ctx.reply({ embeds: [E.ok("💰 Coins Given").addFields({ name: "👤 User", value: `<@${target.id}>`, inline: true }, { name: "💰 Given", value: `${amount} 🪙`, inline: true }, { name: "💳 New Balance", value: `${gE(target.id).coins} 🪙`, inline: true }).setFooter({ text: FT }).setTimestamp()] });
    }
    ...
    case "backup": {
      ...
      if (sub === "create") {
        ...
        return ctx.reply({ embeds: [E.ok("💾 Backup Created").addFields({ name: "🆔 Backup ID", value: bk.id, inline: true }, { name: "💬 Channels", value: `${bk.channels.length}`, inline: true }, { name: "🎭 Roles", value: `${bk.roles.length}`, inline: true }).setFooter({ text: FT }).setTimestamp()] });
      }
      if (sub === "list") {
        const bks = data.serverBackups[guild.id] || [];
        if (!bks.length) return ctx.reply({ embeds: [E.i("No backups found.").setFooter({ text: FT })] });
        return ctx.reply({ embeds: [E.i("💾 Server Backups").setDescription(bks.map(b => `**${b.id}** — ${new Date(b.createdAt).toLocaleString()}`).join("\n")).setFooter({ text: FT })] });
      }
      if (sub === "view") {
        const id = ctx.gs("id") || ctx.ga(1);
        const bk = (data.serverBackups[guild.id] || []).find(b => b.id === id);
        if (!bk) return ctx.reply({ embeds: [E.er("Backup not found.")] }, true);
        return ctx.reply({ embeds: [E.i(`💾 Backup — ${bk.id}`).addFields({ name: "🏠 Server", value: bk.name, inline: true }, { name: "📅 Created", value: new Date(bk.createdAt).toLocaleString(), inline: true }, { name: "💬 Channels", value: `${bk.channels.length}`, inline: true }, { name: "🎭 Roles", value: `${bk.roles.length}`, inline: true }).setFooter({ text: FT })] });
      }
      return ctx.reply({ embeds: [E.w("Subcommands: `create` · `list` · `view <id>`")] }, true);
    }

    default:
      if (ctx.slash) return ctx.reply({ embeds: [E.er("Unknown command.")] }, true);
  }
}
  ...
  try {
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (id === "modal_verify_start") {
        ...
        const rxUser   = await fetchRxUser(username);
        if (!rxUser) return rep(interaction, { embeds: [E.er("Roblox User Not Found").setDescription(`No account found for **${username}**.`)] }, true);
        const code = genCode();
        ...
        saveData();
        return rep(interaction, { embeds: [E.rb("🟣 Step 1 Complete").addFields({ name: "🟣 Roblox Account", value: rxUser.name, inline: true }, { name: "🔑 Your Code", value: `\`${code}\``, inline: true }, { name: "📝 Instructions", value: `1. Go to [Roblox Settings](https://www.roblox.com/my/account) → About tab\n2. Paste: \`${code}\`\n3. Save, then run \`/checkverify ${code}\`` }).setFooter({ text: "Code expires in 5 minutes." }).setTimestamp()] }, true);
      }
    }
  } catch (e) {
    console.error("[Interaction]", e);
    errLog(e, "interactionCreate").catch(() => {});
    try { if (!interaction.replied && !interaction.deferred) await interaction.reply({ content: "❌ Something went wrong.", flags: MessageFlags.Ephemeral }); } catch {}
  }
});
...
client.on("messageCreate", async message => {
  ...
  // Ticket AI
  if (message.channel.name?.startsWith("ticket-") && !tAIOff.has(message.channel.id) && OPENROUTER_KEY) {
    if (aiProcessing.has(message.channel.id)) return;
    const type = message.channel.topic?.match(/^\[(order|partnership|inquiry)\]/)?.[1];
    if (!type || type === "order") return;
  ...
  ctx.gi = n => { const s = ctx.gs(n) || args[0]; const v = parseInt(s); return isNaN(v) ? null : v; };
...
client.on("messageDelete", msg => {
  ...
  if (msg.content) snipeCache.set(msg.channel.id, { content: msg.content, author: msg.author.tag, authorAvatar: msg.author.displayAvatarURL(), timestamp: Date.now() });
});

client.on("messageUpdate", (oldMsg, newMsg) => {
  if (oldMsg.author?.bot || !oldMsg.guild) return;
  if (oldMsg.content && newMsg.content && oldMsg.content !== newMsg.content) {
    editSnipeCache.set(oldMsg.channel.id, { before: oldMsg.content, after: newMsg.content, author: oldMsg.author.tag, authorAvatar: oldMsg.author.displayAvatarURL(), timestamp: Date.now() });
  }
});

process.on("unhandledRejection", e => { console.error("[UnhandledRejection]", e); errLog(e, "unhandledRejection").catch(() => {}); });
process.on("uncaughtException",  e => { console.error("[UncaughtException]", e);  errLog(e, "uncaughtException").catch(() => {}); });
